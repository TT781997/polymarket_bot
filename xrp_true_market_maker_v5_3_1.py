"""
╔══════════════════════════════════════════════════════════════════════════════════╗
║   xrp_true_market_maker_v5.3.1.py                                              ║
║   VERSÃO: 5.3.1 — Shadow Trading + Monte Carlo + PolyMaker LMSR               ║
║                                                                                  ║
║   DELTA vs v5.3.0:                                                               ║
║     [1] §6c PolyMakerBridge: LMSR softmax pricing (C=b·ln(Σe^(qi/b))),        ║
║         Bayesian sequential signal (log-space), EV=p̂−p entry gate.             ║
║     [2] §16 MonteCarloValidator: 5000 sims offline antes do main loop.         ║
║         GO/NO-GO: Sharpe>0.3, median PnL>0, maxDD<30%. Logs [MC_REPORT].      ║
║     [3] Shadow fill engine, deterministic MarketFinder, CB — tudo mantido.     ║
║                                                                                  ║
...
@dataclass
class BotConfig:
    """v5.3.1 — Shadow + MonteCarlo + PolyMaker LMSR."""
    dry_run: bool = True
    ...
    # ── Ficheiros ──────────────────────────────────────────────────────────
    log_file: str = "mm_v531.log"
    audit_file: str = "mm_v531_audit.jsonl"
    config_json_path: str = "config.json"
    config_reload_interval_s: float = 15.0
    # ── Bandas ─────────────────────────────────────────────────────────────
    band_offsets_cents: Tuple[float, ...] = (1.0, 3.0)
    band_sizes_usd: Tuple[float, ...] = (2.0, 1.5)
    # ── Defensivas ─────────────────────────────────────────────────────────
    ...
    def _validate(self):
        ...
        if self.tick_interval_s < 0.5: raise ValueError("tick>=0.5")
...
def init_logging(lf="mm_v531.log") -> logging.Logger:
    ...
    l=logging.getLogger("mm_v531"); l.setLevel(logging.DEBUG); l.handlers.clear()
    ...
    log=l; return l
def _get_log(): return log or logging.getLogger("mm_v531")
...
###############################################################################
class AuditLogger:
    def __init__(self, af="mm_v531_audit.jsonl"):
        self._fh=None
...
class LMSRPricer:
    ...
    def price_up(self) -> float:
        ...
        diff = max(-20.0, min(20.0, diff))  # clamp for numerical stability
...
class BayesianSignal:
    ...
    def update(self, xrp_return: float) -> None:
        ...
        # Likelihood ratio em log-space: log(P(ret|UP)/P(ret|DOWN))
        # Modelo: UP ↔ positive returns, DOWN ↔ negative returns
        # log_lr = ret / std² (simplified Gaussian likelihood ratio)
        log_lr = xrp_return / (self._std ** 2 + 1e-15)
        log_lr = max(-5.0, min(5.0, log_lr))  # Clamp: "NEVER full Kelly on 5min"
        self._log_odds += log_lr
        self._log_odds = max(-10.0, min(10.0, self._log_odds))  # Prevent extreme beliefs
        self._updates += 1
...
class PolyMakerBridge:
    ...
    def compute_skew_adjustment(self, market_mid_up: float) -> float:
        ...
        # Blend: 60% Bayesian (reativo) + 40% LMSR (structural)
        combined_ev = 0.6 * ev_bayes + 0.4 * ev_lmsr

        if abs(combined_ev) < self._cfg.ev_min_entry:
            return 0.0  # Below entry threshold — no adjustment

        # Scale EV to price units (max ±1c)
        skew = combined_ev * 0.10  # 10% of EV as price skew
...
class DualOrderBook:
    ...
    def simulate(self):
        ...
        self.down.ask_levels=[(round(dm+.01*i+sp2/2,2),random.uniform(20,100)) for i in range(5)]
        self.down.stale=False; self.down.last_update_ts=time.time()

###############################################################################
#  §9 — SWEET SPOT PRICER (now with PolyMaker skew)                        #
###############################################################################
@dataclass
class BandQuote:
...
class SweetSpotPricer:
    __slots__=('_cfg','_binance_drift','_polymaker_skew')
    def __init__(self, cfg):
    ...
    def compute_quotes(self, ob, inv_up, inv_down, bk):
        ...
        if not ob.up.is_stale(): q.extend(self._book(ob.up,"UP",inv_up,mi,em))
        if not ob.down.is_stale(): q.extend(self._book(ob.down,"DOWN",inv_down,mi,em))
        return q
    def _sweet(self, levels, mid):
        ...
        return off
    def _book(self, snap, book, inv, mi, em):
        ...
        # v5.3.1: PolyMaker LMSR+Bayesian skew
        ...
        for i,sz in enumerate(cfg.band_sizes_usd):
            ...
            if not sb:
                ...
                if bp<round(am,2): q.append(BandQuote("BUY",book,bp,sz,i))
            if not sa:
                ap=am+af-sk
                if ag: ap=am+max(.01,af*.5)-sk
                ap=round(max(.01,min(.99,ap)),2)
                if ap>round(am,2): q.append(BandQuote("SELL",book,ap,sz,i))
        return q
...
class OrderManager:
    ...
    async def execute_cycle(self, desired, tu, td):
        cfg=self._cfg; nc=0; np_=0; db=cfg.deadband_cents/100
        ...
        for key,q in dk.items():
            if key not in sa:
                ...
                if await self._clob.post_limit_order(tid,q.side,q.price,q.size_usd,q.band_idx,self._audit): np_+=1
        return nc,np_
        ...
        for sim_i in range(n_sims):
            ...
            for _ in range(n_rounds):
                ...
                for _ in range(ticks_per_round):
                    ...
                    # Simulate fill with shadow engine parameters
                    if random.random() < cfg.shadow_fill_prob_base * 0.3:  # ~18% per tick
                        # BUY fill
...
def _log_amm(t_rem,pnl,inv,ob,na,nc,np_,shadow):
    ...
    _get_log().info("%s T-%.0fs | Spr=%s/%s | Inv=%.1f%%/%.1f%% | Ord=%d(C%d/P%d) | F=%d(s:%d) | Reb=%s | Net=%s | CB=%s",
        tag,t_rem,_fc(ob.up.spread),_fc(ob.down.spread),up_pct,dn_pct,na,nc,np_,
...
async def run_round(rn,market,cfg,clob,pricer,omgr,inv,pnl,ob,audit,wallet,
    ...
    while not _shutdown_flag:
        ...
        if cfg.binance_enabled and not _vol_oracle.is_xrp_stale():
            ...
            # v5.3.1: feed PolyMaker Bayesian signal
        ...
        if tick_n%cfg.log_every_n_ticks==0:
            _log_amm(t_rem,pnl,inv,ob,len(clob._active_orders),nc,np_,is_shadow)
...
async def main():
    ...
    _log_sep2()
    _get_log().info("XRP MARKET MAKER v5.3.1 — Shadow + MonteCarlo + PolyMaker")
    ...
    _log_sep2()
    audit.log_event("BOOT",f"v5.3.1|{mode_str}|bk=${bankroll:.4f}")
