# =============================================================================
# BOT XRP POLYMARKET — v4.0.0
# =============================================================================
# CHANGELOG v4.0.0 [Bayesian Signal Processing + LMSR Inefficiency Detection]:
#
# ─────────────────────────────────────────────────────────────────────────────
...
LIVE_TRADING    = False    # v4: mantido como False para comparação de PnL% (embora seja recomendado ativar em produção)
BANKROLL_DEMO   = 10.0     # v4: mantido em $10 para facilitar comparação de PnL% com versões anteriores (mesmo que Kelly agora seja mais conservador)
SLIPPAGE_TOLERANCE = 0.02  # 2 cents slippage tolerance for FOK orders #0.04

# --- AGGRESSIVE ENDGAME ---
AGGRESSIVE_ENDGAME_ACTIVE = True
AGGRESSIVE_ENDGAME_RISK   = 0.03 # % da banca para arriscar no endgame (v4: 2%→3% — slightly more aggressive)
AGGRESSIVE_ENDGAME_S      = 30.0  # trigger de tempo para ativar endgame agressivo (v4: 5.0s→2.0s — muito mais agressivo)
AGGRESSIVE_ENDGAME_MIN_C  = 0.75 # preço mínimo para entrar no endgame agressivo (v4: 75c, mantido — não faz sentido arriscar em preços baixos)
AGGRESSIVE_ENDGAME_MAX_C  = 0.97 # preço máximo para entrar no endgame agressivo (v4: 95c, mantido — não faz sentido arriscar em preços altos)

# --- RISK RULES ---
MAX_MARKET_EXPOSURE = 0.15 # % da banca total que pode estar investida no mercado (v4: mantido em 15% para comparação de PnL%)
MIN_EXPECTED_PROFIT = 0.05 # % mínimo de lucro esperado para entrar em uma posição (v4: mantido em 5% para comparação de PnL%)

# --- MARTINGALE ---
MART_MAX_MULT = 8 # número máximo de martingale (v4: mantido em 8 para comparação de PnL%)

# --- BASE RISK ---
PEG_ARBIT_RISK   = 0.25 # % da banca para arriscar em cada trade de arbitragem de peg (v4: mantido em 25% para comparação de PnL%)
MAX_RISK_PERCENT = 3    # % da banca para arriscar em cada trade de gambling (v4: mantido em 1% para comparação de PnL%)

# --- TOGGLES ---
PEG_ARBIT_ACTIVE   = True  # ativa arbitragem de peg (v4: mantido como True para comparação de PnL%)
GAMBLING_ACTIVE    = True  # ativa trades de gambling (v4: mantido como True para comparação de PnL%)
STOP_LOSS_ACTIVE   = False # ativa stop-loss (v4: mantido como False para comparação de PnL%, embora seja recomendado ativar em produção)
TAKE_PROFIT_ACTIVE = False # ativa take-profit (v4: mantido como False para comparação de PnL%, embora seja recomendado ativar em produção)

# --- PEG ARBIT ---
PA_TRIGGER_SUM  = 0.98       #0.960 # preço combinado (ask_up + ask_down) para disparar a arbitragem de peg (v4: 0.96→0.99 — muito mais conservador, pois agora temos o LMSR para detectar ineficiências)
PA_COOLDOWN     = 0.05       # tempo de cooldown entre trades de peg (v4: mantido em 50ms para comparação de PnL%)
PA_MIN_REM      = 1.0        # tempo mínimo restante para considerar trades de peg (v4: mantido em 1s para comparação de PnL%)
PA_TARGET_BID_C = 0.0        # preço alvo para o bid na arbitragem de peg (v4: mantido em 0 para comparação de PnL%)
MAX_PA_ENTRIES  = 10_000_000 # limite máximo de entradas em peg para evitar loops infinitos (v4: mantido em 10M para comparação de PnL%)

# --- GAMBLING ---
GAMB_START_REM_S  = 300     # tempo restante para começar a considerar trades de gambling (v4: mantido em 300s para comparação de PnL%)
GAMB_CUTOFF_S     = 0       # tempo restante para cortar trades de gambling (v4: mantido em 0s para comparação de PnL%)
GAMB_MIN_EFF_C    = 75.0    # v4: 82→80 (Bayesian filters handle quality)
GAMB_MAX_EFF_C    = 95.0    # v4: mantido em 95 para comparação de PnL% (Bayesian filters should prevent bad entries)
GAMB_BUY_COOLDOWN = 15.0    # tempo de cooldown após uma compra de gambling (v4: mantido em 15s para comparação de PnL%)
GAMB_PEG_MIN      = 0.980   # preço mínimo de peg para considerar trades de gambling (v4: mantido em 0.98 para comparação de PnL%)
GAMB_TARGET_BID_C = 0.0     # preço alvo para o bid nos trades de gambling (v4: mantido em 0 para comparação de PnL%)

# --- FILTERS ---
MAX_SPREAD_CENTS  = 2.20 
BID_ASK_MIN_RATIO = 0.94 # bid/ask ratio mínimo para considerar o mercado saudável (v4: mantido em 0.94 para comparação de PnL%)

# --- HFT ENGINE ---
HFT_WINDOW_SECONDS   = 10  
KALMAN_PROCESS_NOISE = 8e-6 # v4: 1e-5→8e-6 (slightly more responsive to price changes, as Bayesian will handle noise)
KALMAN_MEASURE_NOISE = 4e-3 # v4: 1e-3→4e-3 (acknowledging more noise in 5min markets, but Bayesian will filter it)

# --- GAMBLING ENTRY CONDITIONS ---
GAMB_MAX_VOL_DEV = 0.02      # desvio máximo do volume atual em relação à média (v4: mantido em 0.02 para comparação de PnL%)
GAMB_MAX_ZSCORE  = 1.3       # z-score máximo para considerar o mercado saudável (v4: 1.0→1.3 (slightly relaxed, as Bayesian filters handle quality)
GAMB_MIN_OBI     = 0.75      # v4: 0.80→0.75 (Bayesian compensates)
VPIN_SAFE_LIMIT  = 0.35      # v4: 0.30→0.35 (slightly relaxed)

# --- ENDGAME OVERRIDE ---
ENDGAME_TRIGGER_S    = 30.999 # tempo restante para ativar o endgame override (v4: 30.999s para comparação de PnL%, embora seja recomendado ativar em 30s)
ENDGAME_ZSCORE_LIMIT = 99.0   # v4: 5.0→99.0 (effectively disabling the z-score filter in endgame, as Bayesian should handle it, and we want to be aggressive in endgame)
ENDGAME_VPIN_LIMIT   = 0.70   # v4: 0.50→0.70 (much more aggressive in endgame, as Bayesian should handle it, and we want to be aggressive in endgame)

# --- TAKE-PROFIT ---
TP_SPIKE_ZSCORE     = 4.5  # z-score de volume para detectar spikes de preço e considerar take-profit (v4: 3.0→4.5, mais conservador, pois agora temos o Bayesian para detectar oportunidades de lucro)
TP_MIN_BID_OVER_ASK = 0.05 # diferença mínima entre bid e ask para considerar take-profit (v4: mantido em 0.05 para comparação de PnL%)
TP_MIN_PROFIT_PCT   = 0.05 

# --- STOP-LOSS ---
SL_TOXIC_VPIN   = 0.97 
SL_CRASH_ZSCORE = -5.0 
SL_PANIC_OBI    = 0.02 
SL_BASE_TRIGGER = 0.25 # v4: mantido em 25% para comparação de PnL%, embora seja recomendado ativar em 20% ou 25% para proteção contra flash crashes

# --- KELLY (v4.0.0 — 1/8 Kelly per doc: "NEVER full Kelly on 5min") ---
KELLY_ASSUMED_EDGE = 0.08    # FALLBACK ONLY if Bayesian unavailable
KELLY_FRACTION     = 0.125   # v4: 1/8 Kelly (doc handwritten note)
KELLY_MAX_RISK_PCT = 0.05    # v4: mantido em 5% para comparação de PnL%, embora seja recomendado reduzir para 3-4% em produção, especialmente com o Bayesian fornecendo uma estimativa de edge mais precisa.

# ─────────────────────────────────────────────────────────────────────────────
# BAYESIAN SIGNAL PROCESSING (NEW v4.0.0 — Doc 1)
# ─────────────────────────────────────────────────────────────────────────────
#
#   Sequential Bayesian updating per WS tick.
#   log P(UP|data) += log_likelihood(tick_data | UP)
#   log P(DOWN|data) += log_likelihood(tick_data | DOWN)
#   Normalize to get posterior probabilities.
#
#   Likelihood model uses 3 signals:
#     1. Kalman direction: if kal_up rose → evidence for UP
#     2. OBI dominance: if obi_up > 0.5 → buyers support UP
#     3. VPIN flow: low VPIN on side → healthy, supports that side
#
BAYESIAN_PRIOR           = 0.50   # Uninformative prior each cycle
BAYESIAN_LIKELIHOOD_STD  = 0.015  # Likelihood width (lower=sharper updates)
BAYESIAN_MIN_EDGE        = 0.04   # Min (p_hat - ask) to enter GAMBLING
BAYESIAN_DECAY           = 0.995  # Exponential decay toward prior (prevents overfit)
BAYESIAN_MIN_TICKS       = 10     # Min ticks before trusting posterior

# ─────────────────────────────────────────────────────────────────────────────
# LMSR PRICING (NEW v4.0.0 — Doc 2)
# ─────────────────────────────────────────────────────────────────────────────
#
#   LMSR provides theoretical fair prices via softmax.
#   Inefficiency = divergence between LMSR fair price and market ASK.
#   Used as entry gate: only trade when market is mispriced.
#
LMSR_B                = 100000.0  # Liquidity parameter (Doc 2: b=100,000)
LMSR_INEFF_THRESHOLD  = 0.02      # Min inefficiency to signal entry (2c)

# --- PRODUCTION ---
RATE_LIMIT_CALLS      = 8       # calls per second (v4: mantido em 8 para comparação de PnL%, embora seja recomendado reduzir para 5-6 para evitar riscos de rate limit)
RATE_LIMIT_BURST      = 15      # burst capacity (v4: mantido em 15 para comparação de PnL%, embora seja recomendado reduzir para 10-12 para evitar riscos de rate limit)
MAX_API_RETRIES       = 3       # max retries for API calls (v4: mantido em 3 para comparação de PnL%, embora seja recomendado reduzir para 2 para evitar riscos de rate limit)
BASE_BACKOFF_S        = 1.0     # base backoff time in seconds (v4: mantido em 1s para comparação de PnL%, embora seja recomendado aumentar para 2-3s para evitar riscos de rate limit)
MAX_BACKOFF_S         = 32.0    # max backoff time in seconds (v4: mantido em 32s para comparação de PnL%, embora seja recomendado aumentar para 60s para evitar riscos de rate limit)
BACKOFF_JITTER        = True    # add jitter to backoff to avoid thundering herd (v4: mantido como True para comparação de PnL%, embora seja recomendado manter para evitar riscos de rate limit)
CB_FAIL_THRESHOLD     = 5       # consecutive failures to trigger circuit breaker (v4: mantido em 5 para comparação de PnL%, embora seja recomendado reduzir para 3-4 para produção)
CB_RECOVERY_S         = 60.0    # time to wait before trying API calls again after circuit breaker is triggered (v4: mantido em 60s para comparação de PnL%, embora seja recomendado aumentar para 120s para produção)
WS_RECONNECT_BASE_S  = 1.0      # base time to wait before reconnecting WebSocket (v4: mantido em 1s para comparação de PnL%, embora seja recomendado aumentar para 2-3s para produção)
WS_RECONNECT_MAX_S   = 16.0     # max time to wait before reconnecting WebSocket (v4: mantido em 16s para comparação de PnL%, embora seja recomendado aumentar para 30s para produção)
WS_HEARTBEAT_INTERVAL = 20      # expected interval between WebSocket messages (v4: mantido em 20s para comparação de PnL%, embora seja recomendado reduzir para 15s para detectar falhas mais rapidamente)
WS_HEARTBEAT_TIMEOUT  = 10      # time to wait for a WebSocket message before considering the connection dead (v4: mantido em 10s para comparação de PnL%, embora seja recomendado reduzir para 5-7s para detectar falhas mais rapidamente)

# --- FEES ---
FEE_RATE = 0.25 # 0.25% fee rate (v4: mantido em 0.25% para comparação de PnL%, embora seja recomendado verificar a taxa atual na documentação da Polymarket, pois pode variar entre mercados e condições)
FEE_EXP  = 2 
...
async def logic_loop(m_start, m_end, meta):
    ...
    log_info(
        f"GAMB: start={GAMB_START_REM_S}s | eff=[{GAMB_MIN_EFF_C:.0f}c-{GAMB_MAX_EFF_C:.0f}c] | "
        f"single_side=ON | PA_trigger<={PA_TRIGGER_SUM:.3f}"
    )
    log_sep()
    ...
    while not _shutdown_flag:
        ...
        eff_up, eff_down = eff_price_c(ask_up), eff_price_c(ask_down)
        ...
        if (PEG_ARBIT_ACTIVE and ask_sum <= PA_TRIGGER_SUM and rem > PA_MIN_REM
            ...
            fee_up_c = fee_rate(ask_up) * shares_to_buy * ask_up
            fee_dn_c = fee_rate(ask_down) * shares_to_buy * ask_down
            total_cost = shares_to_buy * ask_up + fee_up_c + shares_to_buy * ask_down + fee_dn_c
            exp_profit = shares_to_buy - total_cost
            exp_pct = (exp_profit / total_cost * 100.0) if total_cost > 0 else 0.0
            log_sep()
            log_m("PEG ARBIT", "ENTRADA",
                f"rem={rstr} | PEG={ask_sum:.4f} underpeg={underpeg_c:.1f}c "
                f"| shares={shares_to_buy:.4f} | cost=${total_cost:.4f} "
                f"| exp_profit=${exp_profit:.4f} ({exp_pct:+.2f}%) | #{pa_count+1}")
            await asyncio.gather(
                open_trade("UP", "PEG ARBIT", rstr, risk=eff_pa_risk, fixed_shares=shares_to_buy, token_id=meta["up"]),
                open_trade("DOWN", "PEG ARBIT", rstr, risk=eff_pa_risk, fixed_shares=shares_to_buy, token_id=meta["down"]))
            log_sep()
            pa_count += 1; last_pa_time = now
            ...
            if rem > GAMB_START_REM_S:
                ...
                if not gamb_started_logged:
                    ...
                    log_m("GAMBLING", "START",
                        f"rem={rstr} | Bayesian+LMSR active | Mart x{mart_multiplier} | single_side=ON")
                ...
                for (g_side, g_ask, g_bid, g_eff, g_z, g_std, g_obi, g_vpin,
                    ...
                    # Single-side lock
                    if gamb_committed_side and gamb_committed_side != g_side:
                        continue
...
async def main():
    ...
    log_info(f"GAMB: start={GAMB_START_REM_S}s eff=[{GAMB_MIN_EFF_C:.0f}-{GAMB_MAX_EFF_C:.0f}c] single_side=ON")
    ...
    while not _shutdown_flag:
        ...
        if best_bids["up"] is not None:
            ...
            if profit_this > 0.00001: total_pnl_pos += profit_this
            elif profit_this < -0.00001: total_pnl_neg += profit_this
