"""
xrp_bot_v9_5_2.py -- Polymarket XRP Up/Down 5-Minute
VERSION: 9.5.2 -- PNL_BOOST + STOPLOSS_15PCT + SETTLEMENT_LOG

Changes from v9.5.1:
1. [PNL_BOOST]   partial_tp_target_net_roi=0.08, partial_tp_fraction=0.80,
                 gamb_min_ask_c=42.0, es_min_threshold=1.60,
                 kelly_max_risk_pct=0.105. More entries, bigger TP.
2. [DYNAMIC_TP]  calculate_dynamic_tp uses Z-score: high-Z → 1.8x ROI target,
                 normal-Z → 1.2x ROI target. Captures momentum better.
3. [EDGE_SIZING] open_trade boosts risk by 25% when edge > 0.25 (cap 0.12).
4. [STOPLOSS]    Hard stop-loss per GAMBLING trade at -15%. Prevents -60%/-70%
                 catastrophic losses. Checked every tick.
5. [HEDGE_FLIP]  adverse_stop_cents=0.9, confirms_needed=3, hedge_max_risk=0.07.
                 More balanced: not too hair-trigger, but still fast.
6. [SETTLE_LOG]  After WINNER detection, logs exact WIN shares @ $1.00,
                 LOSS shares @ $0.00, and total redeemed.
7. [INHERITED]   All v9.5.1 systems: VOL_HEDGE, Shadow Engine, PEG ARB,
                 Bayesian/Kalman, Martingale, logging format unchanged.
"""
...
class BotConfig:
    ...
    gamb_buy_cooldown: float = 6.0          # segundos, cooldown entre compras
    ...
    # ── TAKE PROFIT (v9.5.2: 80% @ +8% net, more aggressive) ───────────
    partial_tp_active: bool = True
    partial_tp_fraction: float = 0.80       # fração, 80% do trade fechado no TP (was 0.70)
    partial_tp_target_net_roi: float = 0.08 # fração, +8% líquido após fees (was 0.05)
    ...
    max_loss_per_trade_pct: float = 0.15     # fração, -15% max loss per GAMBLING trade
...
def calculate_dynamic_tp(
    ...
    """v9.5.2: Dynamic TP target using Z-score for momentum capture.

    High Z (>2.5): target = ask * (1 + base_roi * 1.8)  → hold longer in momentum
    Normal Z:      target = ask * (1 + base_roi * 1.2)  → standard TP
    """
    if trade.ask <= 0.0 or trade.ask >= 0.995:
        return 1.0
    z_abs = abs(z_score)
    if z_abs > 2.5:
        tp_bid = trade.ask * (1.0 + target_net_roi * 1.8)
    else:
        tp_bid = trade.ask * (1.0 + target_net_roi * 1.2)
    if tp_bid >= 0.995:
        return 1.0
    return round(tp_bid, 6)
...
async def logic_loop(
    ...
    log_info(
        f"NOVO CICLO v9.5.2 | {meta['slug']} | LIVE={cfg.live_trading} | "
    ...
    while not ctx.shutdown_flag:
        ...
        if cfg.partial_tp_active and active_trades and \
            ...
            for _tp_t in _tp_candidates:
                ...
                if _tp_bid is None:
                    continue
                # v9.5.2: pass Z-score for momentum-aware TP target
                _tp_z = hft_wins[_tp_t.side].zscore(
                    kalmans[_tp_t.side].x or 0.0
                ) if kalmans[_tp_t.side].x is not None else 0.0
                _tp_target = calculate_dynamic_tp(
                    _tp_t, ctx.fee_cache, cfg.partial_tp_target_net_roi,
                    z_score=_tp_z or 0.0,
                )
                if _tp_target >= 0.995:
                    continue
...
async def main() -> None:
    ...
    log_sep2()
    log_info(
        f"BOT XRP POLYMARKET v9.5.2 -- PNL_BOOST + STOPLOSS_15PCT + SETTLEMENT_LOG | "
    ...
    log_info(
        ...
        f"TP={cfg.partial_tp_fraction:.0%}@+{cfg.partial_tp_target_net_roi:.0%} | "
    ...
    while not ctx.shutdown_flag:
        ...
        if pnl_pct < -5.0:
            await send_alert(
                f"[XRP_BOT v9.5.2] Round loss {pnl_pct:.2f}% | "
                f"bankroll={tsm.state.bankroll} | mart x{tsm.state.mart_level}",
                cfg,
            )
...
def _run_tests() -> None:
    ...
    # Config values — v9.5.2 PNL_BOOST + STOPLOSS_15PCT
    ...
    _assert(cfg.min_vwap_edge == 0.020, "min_vwap_edge=0.020")
    _assert(cfg.max_spread_cents == 3.0, "max_spread_cents=3.0")
    _assert(cfg.bid_ask_min_ratio == 0.955, "bid_ask_min_ratio=0.955")
    ...
    _assert(cfg.max_position_size_usd == 1000.0, "max_position_size_usd=1000.0")
    ...
    _assert(cfg.max_consecutive_losses == 5, "max_consecutive_losses=5")
    _assert(cfg.max_daily_loss_pct == 50.0, "max_daily_loss_pct=50.0")
    _assert(cfg.mart_recovery_factor == 1.22, "mart_recovery_factor=1.22")
    _assert(cfg.peg_arb_active == True, "peg_arb_active=True")
    _assert(cfg.peg_trigger == 0.985, "peg_trigger=0.985")
    _assert(cfg.aggressive_endgame_risk == 0.18, "aggressive_endgame_risk=0.18")
    _assert(cfg.endgame_high_z_risk == 0.24, "endgame_high_z_risk=0.24")
    _assert(cfg.aggressive_endgame_min_c == 0.48, "aggressive_endgame_min_c=0.48")
    _assert(cfg.aggressive_endgame_max_c == 0.995, "aggressive_endgame_max_c=0.995")
    ...
    # v9.5.2: HEDGE_FLIP config (balanced)
    _assert(cfg.adverse_stop_cents == 0.9, "adverse_stop_cents=0.9")
    _assert(cfg.hedge_max_risk_pct == 0.07, "hedge_max_risk_pct=0.07")
    _assert(cfg.hedge_min_prob_opposite == 0.65, "hedge_min_prob_opposite=0.65")
    _assert(cfg.hedge_flip_speed_thresh == 0.002, "hedge_flip_speed_thresh=0.002")
    _assert(cfg.hedge_flip_imbalance_thresh == 0.25, "hedge_flip_imbalance_thresh=0.25")
    _assert(cfg.hedge_flip_confirms_needed == 3, "hedge_flip_confirms_needed=3")
    # v9.5.2: Stop-loss per trade
    _assert(cfg.max_loss_per_trade_pct == 0.15, "max_loss_per_trade_pct=0.15")
    # Shadow latency
    _assert(cfg.shadow_latency_ms == 40.0, "shadow_latency_ms=40.0")

    # v9.5.0: Vol Hedge 1SD-3SD config values
    _assert(cfg.vol_hedge_active == True, "vol_hedge_active=True")
    _assert(cfg.vol_hedge_sd_window == 30, "vol_hedge_sd_window=30")
    _assert(cfg.vol_hedge_1sd_trigger == 1.0, "vol_hedge_1sd_trigger=1.0")
    _assert(cfg.vol_hedge_3sd_target == 3.0, "vol_hedge_3sd_target=3.0")
    _assert(cfg.vol_hedge_no_limit_low == 0.10, "vol_hedge_no_limit_low=0.10")
    _assert(cfg.vol_hedge_no_limit_high == 0.15, "vol_hedge_no_limit_high=0.15")
    _assert(cfg.vol_hedge_abandon_s == 60.0, "vol_hedge_abandon_s=60.0")
    _assert(cfg.vol_hedge_cooldown_s == 8.0, "vol_hedge_cooldown_s=8.0")
    _assert(cfg.vol_hedge_max_risk_pct == 0.10, "vol_hedge_max_risk_pct=0.10")
    _assert(cfg.vol_hedge_min_sd == 0.00005, "vol_hedge_min_sd=0.00005")
    _assert(cfg.vol_hedge_liquidity_min == 5.0, "vol_hedge_liquidity_min=5.0")
    ...
    # Martingale
    tsm_t = TradeStateManager("/tmp/_test_v952.json", Decimal("10.0"))
    ...
    # v9.5.1: ShadowFillEngine has try_fill_sell
    ...
    _assert(_sfe._latency_s == 0.040, f"Shadow latency=40ms (got {_sfe._latency_s*1000:.0f}ms)")

    # v9.5.0: VolatilityHedgeEngine
    vhe = VolatilityHedgeEngine(cfg)
    _assert(hasattr(vhe, 'check_1sd_trigger'), "VolatilityHedgeEngine.check_1sd_trigger exists")
    _assert(hasattr(vhe, 'check_3sd_reached'), "VolatilityHedgeEngine.check_3sd_reached exists")
    _assert(hasattr(vhe, 'check_abandon'), "VolatilityHedgeEngine.check_abandon exists")
    _assert(hasattr(vhe, 'register_entry'), "VolatilityHedgeEngine.register_entry exists")

    # v9.5.0: SD calculation with simulated prices
    for i in range(35):
        vhe.feed_price(2.5 + i * 0.001)
    _assert(vhe.current_sd > 0.0, f"VolHedge SD > 0 (got {vhe.current_sd:.8f})")
    _assert(vhe.current_sd_price > 0.0, f"VolHedge SD_price > 0 (got {vhe.current_sd_price:.8f})")

    # v9.5.0: NO limit price computation (fallback without trade)
    no_lim = vhe.compute_no_limit_price(cfg)
    _assert(0.10 <= no_lim <= 0.15, f"NO limit price fallback in [0.10, 0.15] (got {no_lim})")

    # v9.5.0: Dynamic NO limit price with a mock trade
    _mock_trade = Trade(
        side="DOWN", ask=0.91, bid_at_buy=0.90,
        eff_c=91.5, shares=_d("10.0"), target=None,
        type="VOL_HEDGE_YES", invested_pure=_d("9.1"),
        fee_buy=_d("0.045"), total_out=_d("9.145"),
        token_id="t_test",
    )
    no_lim_dyn = vhe.compute_no_limit_price(cfg, yes_trade=_mock_trade)
    # max_hedge_price = 0.90 - (9.145 / 10.0) = 0.90 - 0.9145 = -0.0145 → clamped to 0.01
    _expected_dyn = round(max(0.01, min(0.20, 0.90 - 9.145 / 10.0)), 4)
    _assert(abs(no_lim_dyn - _expected_dyn) < 1e-6,
            f"Dynamic NO limit price = {_expected_dyn} (got {no_lim_dyn})")

    # v9.5.0: Dynamic price with cheaper trade
    _mock_trade2 = Trade(
        side="UP", ask=0.55, bid_at_buy=0.54,
        eff_c=55.5, shares=_d("10.0"), target=None,
        type="VOL_HEDGE_YES", invested_pure=_d("5.5"),
        fee_buy=_d("0.03"), total_out=_d("5.53"),
        token_id="t_test2",
    )
    no_lim_dyn2 = vhe.compute_no_limit_price(cfg, yes_trade=_mock_trade2)
    # max_hedge_price = 0.90 - (5.53 / 10.0) = 0.90 - 0.553 = 0.347 → clamped to 0.20
    _assert(no_lim_dyn2 <= 0.20, f"Dynamic NO limit clamped to max 0.20 (got {no_lim_dyn2})")

    # v9.5.0: fmt_pnl format verification
    _pnl_pos = fmt_pnl(_d("0.123456"), 5.67)
    _assert("(+)$0.123456" in _pnl_pos, f"fmt_pnl positive format (got {_pnl_pos})")
    _assert("(+5.67%)" in _pnl_pos, f"fmt_pnl positive pct (got {_pnl_pos})")
    _pnl_neg = fmt_pnl(_d("-0.054321"), -2.34)
    _assert("(-)$0.054321" in _pnl_neg, f"fmt_pnl negative format (got {_pnl_neg})")
    _assert("(-2.34%)" in _pnl_neg, f"fmt_pnl negative pct (got {_pnl_neg})")

    # v9.5.0: No active positions initially
    _assert(len(vhe.active_positions) == 0, "No active positions initially")
    _assert(not vhe.has_active_position("UP"), "No active UP position initially")
    _assert(not vhe.has_active_position("DOWN"), "No active DOWN position initially")

    # v9.5.0: Cleanup resets state
    vhe.cleanup_cycle()
    _assert(len(vhe.active_positions) == 0, "Cleanup resets positions")

    # v9.5.2: calculate_dynamic_tp with Z-score
    _tp_trade = Trade(
        side="UP", ask=0.60, bid_at_buy=0.59,
        eff_c=60.5, shares=_d("5.0"), target=None,
        type="GAMBLING", invested_pure=_d("3.0"),
        fee_buy=_d("0.02"), total_out=_d("3.02"),
        token_id="t_tp",
    )
    _tp_normal = calculate_dynamic_tp(_tp_trade, {}, 0.08, z_score=1.0)
    _tp_high_z = calculate_dynamic_tp(_tp_trade, {}, 0.08, z_score=3.0)
    # Normal Z: ask * (1 + 0.08*1.2) = 0.60 * 1.096 = 0.6576
    _assert(abs(_tp_normal - 0.6576) < 0.001,
            f"TP normal Z = 0.6576 (got {_tp_normal:.4f})")
    # High Z: ask * (1 + 0.08*1.8) = 0.60 * 1.144 = 0.6864
    _assert(abs(_tp_high_z - 0.6864) < 0.001,
            f"TP high Z = 0.6864 (got {_tp_high_z:.4f})")
    _assert(_tp_high_z > _tp_normal, "High-Z TP > Normal TP (captures momentum)")
