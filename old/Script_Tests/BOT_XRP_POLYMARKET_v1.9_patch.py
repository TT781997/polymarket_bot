...
# --- v1.9.0: guards de estado para evitar duplicação de tarefas/subscrições ---
_active_subscriptions = set()   # slugs já subscritos no WS
_active_logic_slugs   = set()   # slugs com logic_loop activo
...
async def main():
    ...
    while not _shutdown_flag:
        ...
        # limpeza segura de routing (v1.9)
        for s in list(_per_market_resolved.keys()):
            if _per_market_resolved[s]["event"].is_set():
                _per_market_resolved.pop(s, None)
        ...
        if slug not in _active_subscriptions:
            await _ws_sub_queue.put({
                "assets_ids": [meta["up"], meta["down"]],
                "slug": slug
            })
            _active_subscriptions.add(slug)
        else:
            log_info(f"WS SUB ignorada (já activa) | {slug}")
        log_info(
            f"WS SUB enviada | {slug} | "
            f"up={meta['up'][:12]}... | down={meta['down'][:12]}..."
        )

        log_info("PRICES INIT | aguardando primeiro tick WS")
        try:
            await asyncio.wait_for(price_change.wait(), timeout=5)
            price_change.clear()
            log_info("PRIMEIRO TICK RECEBIDO")
        except asyncio.TimeoutError:
            log_warn("Timeout à espera de tick WS — ciclo ignorado")
            continue

        if best_bids["up"] is not None:
            if slug not in _active_logic_slugs:
                task = asyncio.create_task(
                    logic_loop(start_ts, start_ts + 300, meta, _ws_sub_queue)
                )
                _active_logic_tasks.append(task)
                _active_logic_slugs.add(slug)

                def _cleanup(_):
                    _active_logic_slugs.discard(slug)

                task.add_done_callback(_cleanup)
            else:
                log_warn(f"logic_loop já activa para {slug}")
