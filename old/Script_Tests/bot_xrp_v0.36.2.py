# =============================================================================
# BOT XRP POLYMARKET — v0.36.2
# =============================================================================
# CHANGELOG v0.36.2:
# - Feature: Remoção completa dos módulos Kelly, Avellaneda-Stoikov (incluindo VPIN) e Ciclos (30s/20s) para simplificação e foco em EIGHTY e PEG_ARBIT.
# - Fix: Ajuste no Martingale Condicional para manter explicitamente o estado (multiplicador, acc_loss e rounds) em caso de PnL == 0.0, especialmente se a ronda anterior foi negativa.
# - Bug: Correção de lógica para garantir que PnL zero não reset o sistema se martingale_multiplier > 1.0.
# - Refactor: Limpeza de código, remoção de parâmetros obsoletos e toggles desnecessários.
# - Enhancement: Outputs mais claros nos logs de PnL e Martingale, com tags explícitas para cenários de zero profit.
# =============================================================================
import asyncio
import websockets
import json
...
# ─────────────────────────────────────────────────────────────────────────────
def calc_risk(base: float, mult: float, accum_loss: float, rec_rounds: int, bank: float, extra_dollar: float) -> float:
    """Calcula risco efetivo com Martingale + Recuperação, capped a 15%.
    Fórmula: min(base * mult + accum_loss / rec_rounds / bank + extra_dollar / bank, MAX_RISK_PERCENT=15%)
    """
    if bank <= 0:
        return MAX_RISK_PERCENT
    recovery_bonus = accum_loss / rec_rounds / bank if rec_rounds > 0 else 0
    extra_bonus = extra_dollar / bank
    raw = (base * mult) + recovery_bonus + extra_bonus
    return min(raw, MAX_RISK_PERCENT)
def calc_risk_preview(base: float, mult: float, accum_loss: float, rec_rounds: int, bank: float, extra_dollar: float) -> float:
...
async def logic_loop(
    ...
    active_trades = []
    eff_risk_per_trade = calc_risk(RISK_PER_TRADE, r_mult, accum_loss, rec_rounds, bankroll, extra_dollar)
    ...
    if r_mult > 1.0 or accum_loss > 0 or extra_dollar > 0:
        ...
        extra_bonus_pct = extra_dollar / bankroll if bankroll > 0 else 0.0
        ...
        log_info(
            ...
            f"| extra_bonus=${extra_dollar:.4f}/${bankroll:.4f}={extra_bonus_pct:.1%} "
    ...
    async def open_trade(
        ...
        if trade_type.startswith('CICLO'):
            target = 0.0 / 100.0 if 0.0 > 0 else None
        ...
        if (PEG_ARBIT_ACTIVE
            ...
            if up_in_range and down_in_range:
                ...
                log_m('PEG ARBIT', 'ENTRADA',
                    f"rem={rstr} | PEG={peg_eff:.4f} (margin {margin:.2f}c) | "
                ...
                if peg_arbit_count == 0:
                    reasons = []
                    if not up_in_range:
                        reasons.append(f"UP {eff_up_c:.1f}c fora [0-45] e [55-99.9]")
                    if not down_in_range:
                        reasons.append(f"DOWN {eff_down_c:.1f}c fora [0-45] e [55-99.9]")
                    log_m('PEG ARBIT', 'SKIP',
                        f"rem={rstr} | PEG OK ({peg_eff:.4f}) mas {' | '.join(reasons)}")
    # =====================================================================
    # 2. TARGET CHECK + STOP-LOSS (BID PURO — como pediste)
    # =====================================================================
    for trade in active_trades[:]:
        s_side = trade['side']
        ...
        # Proteção contra wide-spread (sem mudar para MID)
                ...
                for e_side, nom in (('UP', u_ask), ('DOWN', d_ask)):
                    ...
                    if eighty_tick_count[e_side] >= EIGHTY_MIN_TICKS:
                        if peg_eff * 100.0 < EIGHTY_PEG_MIN_C:
                            eighty_reset(e_side, rstr,
                                f"PEG {peg_eff*100:.1f}c < min {EIGHTY_PEG_MIN_C:.1f}c")
                            continue
...
async def main():
    ...
    global martingale_multiplier, accumulated_loss, recovery_rounds_remaining, extra_dollar_risk
    martingale_multiplier = 1.0
    accumulated_loss = 0.0
    recovery_rounds_remaining = 1
    extra_dollar_risk = 0.0
    log_sep2()
    log_info("BOT XRP POLYMARKET v0.36.2 INICIADO")
    log_sep()
    ...
    log_info(" Regras:")
    log_info(" - PnL < 0: mult x2 | +10 rounds | acc_loss += loss | extra_stake += 0.5*loss")
    ...
    while True:
        ...
        if last_day is None or market_day != last_day:
            ...
            log_info(f" Martingale : x{martingale_multiplier:.0f} | acc_loss=$0.0000 | extra_stake=$0.0000 | rounds={recovery_rounds_remaining}")
            log_sep2()
        ...
        if best_asks['up'] is not None:
            ...
            if profit_this < 0:
                # Loss: dobra multiplicador, adiciona 10 rounds, soma 100% da perda ao acc_loss, soma 50% ao extra_dollar_risk
                ...
                next_risk_eighty = calc_risk_preview(EIGHTY_RISK, martingale_multiplier, accumulated_loss, recovery_rounds_remaining, bankroll, extra_dollar_risk)
                next_risk_peg = calc_risk_preview(PEG_ARBIT_RISK, martingale_multiplier, accumulated_loss, recovery_rounds_remaining, bankroll, extra_dollar_risk)
                ...
                log_info(
                    ...
                    f"| Acc_loss: ${accumulated_loss:.4f} (+${loss:.4f}) "
                    f"| Extra_stake: ${extra_dollar_risk:.4f} (+${loss * 0.5:.4f}) "
                ...
                # Green: reseta mult para x1 e recupera acc_loss e extra_dollar_risk
                prev_accum = accumulated_loss
                accumulated_loss = max(0.0, accumulated_loss - profit_this)
                recovered_accum = prev_accum - accumulated_loss
                prev_extra = extra_dollar_risk
                extra_dollar_risk = max(0.0, extra_dollar_risk - profit_this)
                recovered_extra = prev_extra - extra_dollar_risk
                martingale_multiplier = 1.0
                recovery_rounds_remaining = max(1, recovery_rounds_remaining - 1)
                log_str = f"MARTINGALE CONDICIONAL | PnL > 0 (Green) | Mult reset x1"
                if recovered_accum > 0:
                    log_str += f" | RECUPERAÇÃO ACC (recuperados ${recovered_accum:.4f}"
                    if accumulated_loss > 0:
                        log_str += f" | restam ${accumulated_loss:.4f})"
                    else:
                        log_str += ") | ACC completo"
                if recovered_extra > 0:
                    log_str += f" | RECUPERAÇÃO EXTRA (recuperados ${recovered_extra:.4f}"
                    if extra_dollar_risk > 0:
                        log_str += f" | restam ${extra_dollar_risk:.4f})"
                    else:
                        log_str += ") | EXTRA completo"
                log_str += f" | Rounds restantes: {recovery_rounds_remaining}"
                log_info(log_str)
                log_info(f"ROUND | {pnl_str}")
            if daily_profit == 0.0:
            ...
            log_info(
                ...
                f"Extra Stake: ${extra_dollar_risk:.4f} | "
