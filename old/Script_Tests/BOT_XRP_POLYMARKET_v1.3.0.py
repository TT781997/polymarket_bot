# =============================================================================
# BOT XRP POLYMARKET — v1.3.0
# =============================================================================
#
# CHANGELOG v1.3.0  [3 alterações cirúrgicas]:
#
# [1] STOP-LOSS — parâmetros e lógica ajustados (Anti Flash-Crash 5s):
#     SL_THRESHOLD = 0.35  (era 0.30) — inicia contagem aos 35c
#     SL_TICKS     = 10    (era 6)    — 10 ticks consecutivos
#     SL_CHECK_S   = 0.5   (era 2.0)  — intervalo 0.5s → total 5s confirmação
#     Lógica: qualquer tick acima do threshold faz reset imediato a 0.
#     A queda tem de ser 100% contínua e sustentada ao longo dos 10 ticks.
#
# [2] TREND — removida pesquisa de mercado XRP 1h; usa token do ciclo actual:
#     Removida função find_1h_xrp_up_token() inteira.
#     Removida dependência de get_markets() que gerava WARNs nos logs.
#     xrp_1h_token_up é agora alimentado por meta['up'] em main() a cada ciclo.
#     trend_update_task usa directamente o token disponível sem pesquisa.
#     TREND_INTERVAL alterado para "1h" com TREND_FIDELITY=60.
#     fetch_trend_from_clob() inalterada — continua a funcionar igual.
#
# [3] PRICES INIT — removido fetch REST inicial; WS é a única fonte de preços:
#     Removida função fetch_initial_prices_sdk() inteira.
#     Removida chamada await fetch_initial_prices_sdk() em main().
#     best_bids e best_asks iniciam a None por defeito (comportamento anterior).
#     WS preenche os preços no primeiro tick recebido do orderbook.
#     Elimina erros PolyApiException em mercados recém-nascidos.
#
# =============================================================================
...
#     [REMOVIDA em v1.3.0 — substituída por WS exclusivo]
#
...
#     find_1h_xrp_up_token(): [REMOVIDA em v1.3.0]
#
# =============================================================================
# CHANGELOG v1.1.0:
...
# --- RISCO BASE ---
GAMBLING_RISK     = 0.05  # Risco base Gambling: fraccao da banca actual por trade [Range: 0.01 | 0.50]
PEG_ARBIT_RISK    = 0.30  # Risco base Peg Arbit: fraccao da banca (budget total ambos os lados) [Range: 0.01 | 0.50]
MAX_RISK_PERCENT  = 0.50  # Hard cap INVIOLAVEL: investimento total nunca excede 20% da banca [Range: 0.10 | 0.50]
...
# --- TREND 1H XRP ---
# v1.3.0: TREND_INTERVAL alterado para "1h"; token alimentado por meta['up']
# de cada ciclo — sem pesquisa de mercado separado.
TREND_UPDATE_S  = 300.0   # Intervalo de actualizacao do trend (seg) [Range: 30.0 | 300.0]
...
# --- STOP-LOSS (Anti-Dump / Flash-Crash com confirmacao de 5 segundos) ---
# v1.3.0: Parâmetros ajustados para confirmação sustentada de 5 segundos.
#   Matematica: 10 ticks * 0.5s = 5 segundos de queda contínua confirmada.
#   Qualquer tick acima de SL_THRESHOLD faz reset imediato a 0.
#   A queda tem de ser 100% contínua e sustentada ao longo dos 10 ticks.
SL_THRESHOLD = 0.35  # BID abaixo deste nivel (35c) inicia contagem Anti-Dump [Range: 0.01 | 0.50]
SL_TICKS     = 10    # Ticks consecutivos abaixo do threshold para confirmar (10 x 0.5s = 5s) [Range: 1 | 20]
SL_CHECK_S   = 0.5   # Intervalo de verificacao do SL (seg) — 0.5s por tick [Range: 0.1 | 5.0]
...
# Trend XRP 1h
# v1.3.0: xrp_1h_token_up alimentado por meta['up'] em main() — sem pesquisa.
xrp_1h_trend    = 'NEUTRAL'  # UP / DOWN / NEUTRAL [Range: str]
xrp_1h_token_up = None       # Token ID UP do ciclo actual (actualizado por main()) [Range: None | str]
...
try:
    ...
    # Cliente read-only sem chave — para get_prices_history e get_spread públicos
...
async def ws_handler(t_up: str, t_down: str):
    ...
    v1.3.0: WS e a UNICA fonte de precos iniciais.
    best_bids e best_asks sao None ate ao primeiro tick recebido.
...
async def logic_loop(
    ...
    STOP-LOSS CIRÚRGICO (v1.2.1 + v1.3.0):
              ...
              v1.3.0: SL_THRESHOLD=0.35, SL_TICKS=10, SL_CHECK_S=0.5
              (10 ticks * 0.5s = 5 segundos de confirmação sustentada)
    """
    ...
    if STOP_LOSS_ACTIVE:
        mods.append(
            f"STOP_LOSS(<{SL_THRESHOLD:.2f}/{SL_TICKS}ticks*{SL_CHECK_S:.1f}s"
            f"={SL_TICKS*SL_CHECK_S:.0f}s)"
        )
    ...
    # =========================================================================
    # STOP-LOSS TASK — Anti-Dump / Flash-Crash com confirmação de 5 segundos
    # =========================================================================
    async def stop_loss_task():
        """
        v1.3.0: Parâmetros actualizados para Anti Flash-Crash de 5 segundos.
          SL_THRESHOLD = 0.35 (35c) — nível de início de contagem
          SL_TICKS     = 10         — ticks consecutivos necessários
          SL_CHECK_S   = 0.5        — intervalo entre ticks (0.5s cada)
          Total confirmação: 10 * 0.5s = 5 segundos de queda sustentada

        Lógica de reset: qualquer único tick com BID >= SL_THRESHOLD faz
        reset IMEDIATO dos contadores a 0. A queda tem de ser 100% contínua
        e sustentada ao longo das 10 verificações para disparar a venda.

        Anti-Dump: tick só conta se BID < SL_THRESHOLD E mid < SL_MID_MAX.
        ...
        while True:
                ...
                if sl_valid:
                    ...
                    if not sl_started[side]:
                        ...
                        log_m('STOPLOSS', 'MONITOR',
                            ...
                            f"| confirmacao em {(SL_TICKS - sl_ticks[side]) * SL_CHECK_S:.1f}s"
                        )
                    else:
                    ...
                    # v1.3.0: reset IMEDIATO a 0 se qualquer tick recuperar acima do threshold
                    if sl_ticks[side] > 0:
                        reason = (
                            f"bid={bid_c:.1f}c voltou acima {SL_THRESHOLD*100:.1f}c "
                            f"(RESET IMEDIATO ao tick {sl_ticks[side]}/{SL_TICKS})"
            ...
            if triggered:
                log_m('STOPLOSS', 'TRIGGER',
                    ...
                    f"| {SL_TICKS} ticks consecutivos confirmados "
                    f"({SL_TICKS * SL_CHECK_S:.0f}s de queda sustentada)"
                )
...
async def main():
    ...
    # ─ Log de arranque ───────────────────────────────────────────────────────
    log_sep2()
    log_info(f"BOT XRP POLYMARKET v1.3.0 INICIADO")
    log_sep2()
    ...
    log_info(f"   best_bids/best_asks iniciam a None")
    log_info(f"   WS preenche os precos no primeiro tick do orderbook")
    log_info(f"   Elimina erros PolyApiException em mercados recem-nascidos")
    log_sep2()
    log_info("TREND (v1.3.0 — sem pesquisa de mercado):")
    log_info(f"   xrp_1h_token_up alimentado por meta['up'] a cada ciclo")
    ...
    log_sep2()
    log_info("STOP LOSS CIRURGICO (v1.3.0 — Anti Flash-Crash 5 segundos):")
    log_info(f"   SL_THRESHOLD = {SL_THRESHOLD:.2f} ({SL_THRESHOLD*100:.0f}c) — inicia contagem")
    log_info(f"   SL_TICKS     = {SL_TICKS} ticks consecutivos obrigatorios")
    log_info(f"   SL_CHECK_S   = {SL_CHECK_S}s por tick")
    log_info(f"   Total: {SL_TICKS} x {SL_CHECK_S}s = {SL_TICKS*SL_CHECK_S:.0f}s de confirmacao sustentada")
    log_info(f"   Reset IMEDIATO a 0 se qualquer tick recuperar acima de {SL_THRESHOLD*100:.0f}c")
    ...
    while True:
        ...
        # xrp_1h_token_up é actualizado aqui — a trend_update_task vai usá-lo
        # na próxima iteração do seu loop de TREND_UPDATE_S segundos.
        xrp_1h_token_up = meta['up']
        log_info(f"TREND TOKEN | ciclo={slug} | token_up={meta['up'][:16]}... (sem pesquisa de mercado)")

        # Reset estado WS
        # v1.3.0: best_bids/best_asks iniciam a None — WS preenche no primeiro tick.
        ...
        # v1.3.0: fetch_initial_prices_sdk() REMOVIDO.
        # Precos iniciais agora fornecidos exclusivamente pelo WS.
        # Aguarda primeiro tick WS antes de entrar no logic_loop.
