...
async def logic_loop(m_start: float, m_end: float, meta: dict):
    ...
    while not _shutdown_flag:
        ...
        if rem <= 0.0:
            ...
            if active_trades:
                log_info(
                    f"Aguardando resolucao WS oficial "
                    f"(max {RESOLVE_MAX_WAIT_S:.0f}s — real-time settlement v1.8.0)..."
                )
                try:
                    await asyncio.wait_for(
                        resolved_event.wait(),
                        timeout=RESOLVE_MAX_WAIT_S
                    )
                    ...
                    # v1.8.0: NÃO estima vencedor por BID — REFUND e continua
                    log_warn(
                        f"CRITICO: {RESOLVE_MAX_WAIT_S:.0f}s sem market_resolved WS — "
                        f"PnL NAO registado (real-time settlement exige resolucao oficial)"
                    )
                    for trade in active_trades[:]:
                        # Devolver total_out à banca — operação neutra
                        bankroll += trade["total_out"]
                        log_warn(
                            f"REFUND | {trade['side']} {trade['type']} "
                            f"| total_out=${trade['total_out']:.4f} devolvido a banca "
                            f"| sem resolucao oficial — PnL nao registado"
                        )
                        active_trades.remove(trade)
            break
...
# =============================================================================
# MAIN (v1.8.0 — graceful shutdown + real-time settlement)
# =============================================================================
...
async def main():
    ...
    # ─ Log de arranque ───────────────────────────────────────────────────────
    log_sep2()
    log_info("BOT XRP POLYMARKET v1.8.0 — WS FULL-STREAM + REAL-TIME SETTLEMENT")
    log_sep2()
    ...
    log_sep2()
    log_info("v1.8.0 — ALTERAÇÕES ARQUITECTURAIS:")
    log_info(f"   WS Full-Stream  : zero REST no hot path (spread calculado de WS)")
    log_info(f"   Real-Time Settle: RESOLVE_MAX_WAIT_S={RESOLVE_MAX_WAIT_S:.0f}s | sem fallback BID")
    log_info(f"   Strict Transit  : sem overlap de mercados | aguarda resolucao oficial")
    log_info(f"   WS Logger       : websockets.client=WARNING (sem < TEXT ... >)")
    log_sep2()
    ...
    while not _shutdown_flag:
        ...
        # v1.8.0: cancela WS APÓS resolução completa (sem overlap)
        ws_task.cancel()
        try:
            await ws_task
        except asyncio.CancelledError:
