# =============================================================================
# BOT XRP POLYMARKET — v1.6.0
# =============================================================================
#
# CHANGELOG v1.6.0  [6 Refactorings Cirúrgicos — Win Rate 95%]:
#
# [1] MOTOR HFT ULTRA-REATIVO (10s):
#     HFT_WINDOW_SECONDS mantido em 10s (já configurado em v1.5.0).
#     GAMB_MAX_VOL_DEV  : 0.10 -> 0.04 (squeeze mais apertado — menos ruído)
#     GAMB_MAX_ZSCORE   : 1.8  -> 1.1  (anti-topo mais sensível)
#     VPIN_SAFE_LIMIT   : 0.55 -> 0.40 (fluxo normal mais restritivo em modo normal)
#
# [2] MICRO-TREND 30s (novo — class MicroTrendTracker):
#     Divide a macro-janela de 1h em micro-ciclos de 30 segundos.
#     Acumula preços Kalman por lado (UP/DOWN) numa rolling window de 30s.
#     Compara média do primeiro terço vs. último terço para determinar direcção.
#     Log periódico a cada MICRO_TREND_LOG_INTERVAL_S (30s) — apenas informativo.
#     Parâmetros: MICRO_TREND_WINDOW_S=30.0, MICRO_TREND_THRESHOLD=0.002,
#                 MICRO_TREND_LOG_INTERVAL_S=30.0.
#     Não bloqueia entradas — diagnóstico para o operador saber o momentum.
#     Criado dentro de logic_loop (reset automático a cada ciclo de 5min).
#
# [3] MODO AGRESSIVO FINAL — Endgame Override (Últimos 30s):
#     Quando remaining_seconds <= 30.999:
#       Cond 2 (Z-Score): limite temporário -> 99.0 (desativado — aceita picos)
#       Cond 4 (VPIN):    limite temporário -> 0.60 (relaxado vs. VPIN_SAFE_LIMIT)
#     Globals GAMB_MAX_ZSCORE e VPIN_SAFE_LIMIT nunca são mutados (thread-safe).
#     Logs diferenciam [ENDGAME(x.xx)] vs [NORMAL(x.xx)] em BLOCK/WARN/ENTRADA.
#
# [4] GAP DE LUCRO (Entry vs TP):
#     GAMB_MAX_ZSCORE : 1.8  -> 1.1  (entrada conservadora — ver [1])
#     TP_SPIKE_ZSCORE : 3.0         (mantido — gap Z: entrada=1.1 vs TP=3.0)
#     GAMB_MAX_EFF_C  : 95.0 -> 78.0 (bloqueia entradas acima de 78c efectivos)
#
# [5] STOP-LOSS ANTI-PÂNICO:
#     SL_BASE_TRIGGER : 0.35 -> 0.25 (linha de perigo mais baixa — menos pânico)
#     SL_PANIC_OBI    : 0.05 -> 0.02 (colapso real = apenas 2% compradores)
#
# [6] GESTÃO DE RISCO — Compensação:
#     KELLY_FRACTION     : 0.20 -> 0.08 (posições menores — maior segurança)
#     KELLY_MAX_RISK_PCT : 0.08 -> 0.05 (cap de 5% por trade)
#
# FUNÇÕES/CLASSES AFECTADAS:
#   - Parâmetros globais (secção PARAMETROS CONFIGURÁVEIS): 8 valores alterados,
#     3 novos parâmetros MICRO_TREND adicionados.
#   - class MicroTrendTracker (nova — inserida após VPINTracker)
#   - logic_loop(): micro_trend_trackers dict, last_microtrend_log, endgame
#     override (_eff_zscore_limit / _eff_vpin_limit), Cond 2 / Cond 4 actualizadas,
#     log periódico de micro-trend, mods header e log de arranque do ciclo.
#   - main(): startup log actualizado para novos parâmetros e novos módulos.
#
# =============================================================================
...
GAMB_MAX_EFF_C    = 78.0    # eff_c maximo para entrada (cents); v1.6.0: 95.0->78.0 bloqueia entradas acima de 78c [Range: 65.0 | 99.9]
...
# --- MICRO-TREND (Análise de micro-ciclos de 30s — v1.6.0) ---
#
#   Divide a macro-janela de 1h em micro-ciclos de MICRO_TREND_WINDOW_S segundos.
#   Acumula preços Kalman por lado (UP/DOWN) e calcula a direcção de curto prazo
#   comparando a média do primeiro terço vs. a média do último terço da janela.
#   Apenas informativo — não bloqueia entradas Gambling.
#   Log periódico a cada MICRO_TREND_LOG_INTERVAL_S segundos no ficheiro de log.
#
MICRO_TREND_WINDOW_S       = 30.0   # Janela de acumulação de preços Kalman para micro-trend (seg) [Range: 10.0 | 120.0]
MICRO_TREND_THRESHOLD      = 0.002  # Delta mínimo entre terços para classificar UP/DOWN (evita ruído) [Range: 0.001 | 0.010]
MICRO_TREND_LOG_INTERVAL_S = 30.0   # Intervalo de log periódico da micro-tendência (seg) [Range: 10.0 | 60.0]

# --- MICRO-REGIMES & GAMBLING ENTRY (4 condicoes) ---
#
#   Cond 1 — Regime de compressao (Squeeze):
GAMB_MAX_VOL_DEV   = 0.04   # StdDev(Kalman,10s) <= 4c: regime estavel/comprimido; v1.6.0: 0.10->0.04 [Range: 0.01 | 0.15]
#   Cond 2 — Anti-pico (Z-Score):
GAMB_MAX_ZSCORE    = 1.1    # Z <= 1.1: preco nao esta num pico vs. trajectoria Kalman; v1.6.0: 1.8->1.1 [Range: 0.5 | 3.0]
...
VPIN_SAFE_LIMIT    = 0.40   # VPIN <= 0.40: fluxo normal; v1.6.0: 0.55->0.40 — sem dump institucional detetado [Range: 0.30 | 0.95]
...
#   Globals GAMB_MAX_ZSCORE e VPIN_SAFE_LIMIT NUNCA são mutados (thread-safe).
#   Tags [ENDGAME(x.xx)] e [NORMAL(x.xx)] visíveis em todos os logs BLOCK/WARN/ENTRADA.
#
...
TP_SPIKE_ZSCORE    = 3.0    # Vende GAMBLING imediatamente se Z(Kalman) >= 3.0 (wick absurdo); gap vs. entrada=1.1 [Range: 1.5 | 4.0]
...
SL_PANIC_OBI       = 0.02   # OBI <= 2%: colapso real confirmado — compradores abandonaram; v1.6.0: 0.05->0.02 [Range: 0.01 | 0.50]
SL_BASE_TRIGGER    = 0.25   # BID <= 25c activa a verificacao dos triggers A/B/C; v1.6.0: 0.35->0.25 anti-panico [Range: 0.10 | 0.60]
...
KELLY_FRACTION     = 0.08   # Fractional Kelly: usar ~1/12 de Kelly para seguranca maxima; v1.6.0: 0.20->0.08 [Range: 0.05 | 1.0]
KELLY_MAX_RISK_PCT = 0.05   # Cap absoluto de risco por trade Kelly (5% da banca); v1.6.0: 0.08->0.05 [Range: 0.01 | 0.25]
...
def calc_kelly_risk(ask: float) -> float:
    ...
    Interpretacao: a p=0.87 ask, com edge=5%, kelly ~ 4-5% → frac ~ 0.32-0.40%.
...
class VPINTracker:
    ...
    Interpretacao:
      ...
      >= SL_TOXIC_VPIN   (0.95): dump/pump institucional — SL imediato.
...
# =============================================================================
# MICRO-TREND TRACKER — Análise de Micro-Ciclos de 30s (v1.6.0)
# =============================================================================

class MicroTrendTracker:
    """
    Análise de micro-tendência em janelas deslizantes de MICRO_TREND_WINDOW_S segundos.

    Divide a macro-janela de 1h em micro-ciclos de 30 segundos, acumulando preços
    Kalman para determinar a direcção de curto prazo do mercado antes de cada entrada
    Gambling. Permite ao operador saber para onde o mercado está a cair/subir.

    Metodologia:
      Acumula (timestamp, kal_price) numa deque com expiração automática (30s).
      Compara a média do primeiro terço vs. média do último terço da janela.
        Delta > +MICRO_TREND_THRESHOLD  -> "UP"    (mercado a subir)
        Delta < -MICRO_TREND_THRESHOLD  -> "DOWN"  (mercado a cair)
        Caso contrário                  -> "NEUTRAL" (consolidação)

    Requisito mínimo: 6 pontos na janela para divisão em terços significativa.

    Uso:
      mt = MicroTrendTracker(window_s=30.0, threshold=0.002)
      mt.add(kal_price, now)                       # actualiza a cada tick
      direction, delta, n = mt.trend()             # ('UP'|'DOWN'|'NEUTRAL', float, int)

    Integração em logic_loop:
      Actualizado a cada tick após update Kalman/HFT/VPIN.
      Log periódico a cada MICRO_TREND_LOG_INTERVAL_S segundos.
      Apenas informativo — não bloqueia entradas Gambling.
      Criado dentro de logic_loop (reset automático por ciclo de 5min).

    Parâmetros configuráveis:
      MICRO_TREND_WINDOW_S       = 30.0   — janela de acumulação (seg)
      MICRO_TREND_THRESHOLD      = 0.002  — delta mínimo para UP/DOWN
      MICRO_TREND_LOG_INTERVAL_S = 30.0   — intervalo de log periódico
    """
    __slots__ = ("window_s", "threshold", "data")

    def __init__(self, window_s: float = 30.0, threshold: float = 0.002):
        self.window_s:  float = window_s
        self.threshold: float = threshold
        self.data:      deque = deque()  # deque de (timestamp, kal_price)

    def add(self, kal_price: float, ts: float):
        """
        Adiciona preço Kalman com timestamp e expira registos mais antigos que window_s.
        """
        self.data.append((ts, kal_price))
        cutoff = ts - self.window_s
        buf    = self.data
        while buf and buf[0][0] < cutoff:
            buf.popleft()
    ...
    def trend(self):
        """
        Calcula a micro-tendência actual.
        Retorna (direction, delta, n_points):
          direction : "UP" | "DOWN" | "NEUTRAL"
          delta     : variação média último terço - primeiro terço (float)
          n_points  : número de pontos na janela actual (int)
        Requer mínimo 6 pontos; abaixo disso retorna ("NEUTRAL", 0.0, n).
        """
        n = len(self.data)
        if n < 6:
            return "NEUTRAL", 0.0, n
        prices    = [p for _, p in self.data]
        third     = max(1, n // 3)
        first_avg = sum(prices[:third]) / third
        last_avg  = sum(prices[-third:]) / third
        delta     = last_avg - first_avg
        if   delta >  self.threshold: direction = "UP"
        elif delta < -self.threshold: direction = "DOWN"
        else:                         direction = "NEUTRAL"
        return direction, delta, n

    def reset(self):
        """Reinicia o tracker — criado fresh em logic_loop; método disponível para uso externo."""
        self.data.clear()
...
async def logic_loop(m_start: float, m_end: float, meta: dict):
    ...
    GAMBLING — Motor Quantitativo HFT (v1.5.0 / v1.6.0):
               4 condicoes simultaneas obrigatorias:
               1. std(Kalman,10s) <= GAMB_MAX_VOL_DEV  (regime comprimido)
               2. Z(Kalman)       <= GAMB_MAX_ZSCORE    (nao em pico; v1.6.0: 1.1)
               3. OBI             >= GAMB_MIN_OBI        (compradores dominam)
               4. VPIN            <= VPIN_SAFE_LIMIT     (fluxo saudavel; v1.6.0: 0.40)
               Tamanho: calc_kelly_risk(ask) — sem Martingale.
               eff_c maxima: GAMB_MAX_EFF_C=78.0c (v1.6.0: 95.0->78.0).

    ENDGAME OVERRIDE (v1.6.0 — Últimos 30.999s):
               Quando remaining <= ENDGAME_TRIGGER_S:
               Cond 2: Z_limit -> ENDGAME_ZSCORE_LIMIT (99.0 = desativado)
               Cond 4: VPIN_limit -> ENDGAME_VPIN_LIMIT (0.60 = relaxado)
               Globals nunca mutados — variáveis locais por tick.

    MICRO-TREND (v1.6.0 — Informativo):
               MicroTrendTracker por lado acumula preços Kalman em rolling 30s.
               Log a cada MICRO_TREND_LOG_INTERVAL_S: UP/DOWN/NEUTRAL + delta.
               Não bloqueia entradas — diagnóstico para o operador.

    TAKE-PROFIT DINAMICO (v1.5.0):
               Se Z(Kalman) >= TP_SPIKE_ZSCORE (3.0): vende GAMBLING ao BID.
               Gap entrada (1.1) vs. TP (3.0) garante lucro antes de reversão.

    STOP-LOSS (v1.5.0/v1.6.0 — OR logic):
               BID <= SL_BASE_TRIGGER (0.25) E qualquer trigger:
               A. VPIN >= SL_TOXIC_VPIN   (dump institucional)
               B. Z   <= SL_CRASH_ZSCORE  (crash Kalman -4sigma)
               C. OBI <= SL_PANIC_OBI     (colapso real — apenas 2% compradores)
    ...
    mods.append(f"MICRO_TREND({MICRO_TREND_WINDOW_S:.0f}s/Δ>={MICRO_TREND_THRESHOLD:.3f})")
    mods.append(f"ENDGAME_OVERRIDE(<=30.999s:Z→{ENDGAME_ZSCORE_LIMIT:.0f}/VPIN→{ENDGAME_VPIN_LIMIT:.2f})")
    ...
    log_info(
        f"MICRO-TREND: Window={MICRO_TREND_WINDOW_S:.0f}s "
        f"threshold={MICRO_TREND_THRESHOLD:.3f} "
        f"log_interval={MICRO_TREND_LOG_INTERVAL_S:.0f}s (informativo — nao bloqueia)"
    )
    log_info(
        f"ENDGAME OVERRIDE: trigger=rem<={ENDGAME_TRIGGER_S:.3f}s "
        f"| Z_limit={GAMB_MAX_ZSCORE:.1f}->{ENDGAME_ZSCORE_LIMIT:.0f}(desativ.) "
        f"| VPIN_limit={VPIN_SAFE_LIMIT:.2f}->{ENDGAME_VPIN_LIMIT:.2f}(relaxado)"
    )
    ...
    # =========================================================================
    # ESTADO MICRO-TREND — MicroTrendTracker por lado (v1.6.0)
    # =========================================================================
    micro_trend_trackers = {
        "UP":   MicroTrendTracker(MICRO_TREND_WINDOW_S, MICRO_TREND_THRESHOLD),
        "DOWN": MicroTrendTracker(MICRO_TREND_WINDOW_S, MICRO_TREND_THRESHOLD)
    }
    last_microtrend_log: float = 0.0  # Unix ts do ultimo log de micro-trend
    ...
    while True:
        ...
        # ── Micro-Trend — actualiza trackers a cada tick (v1.6.0) ────────────
        # Acumula preços Kalman para análise de direcção em rolling 30s.
        # Log periódico gerido abaixo (após tick log).
        micro_trend_trackers["UP"].add(kal_up,   now)
        micro_trend_trackers["DOWN"].add(kal_down, now)
        ...
        # ── Micro-Trend — log periódico a cada MICRO_TREND_LOG_INTERVAL_S ────
        # (v1.6.0) Informa a direcção do mercado a cada 30s para diagnóstico.
        # Log mostra: lado, direcção, delta, pontos na janela, Kalman actual.
        # Apenas informativo — não bloqueia nem condiciona entradas Gambling.
        if now - last_microtrend_log >= MICRO_TREND_LOG_INTERVAL_S:
            last_microtrend_log = now
            for _mt_side in ("UP", "DOWN"):
                _mt_dir, _mt_delta, _mt_n = micro_trend_trackers[_mt_side].trend()
                _mt_kal_x = kalmans[_mt_side].x
                _mt_kal_s = fc(_mt_kal_x) if _mt_kal_x is not None else "n/a"
                log_m("MICRO-TREND", _mt_side,
                    f"rem={rstr} | dir={_mt_dir} delta={_mt_delta:+.5f} "
                    f"pts={_mt_n}/{MICRO_TREND_WINDOW_S:.0f}s "
                    f"thresh={MICRO_TREND_THRESHOLD:.3f} | Kalman={_mt_kal_s}"
                )

        # =====================================================================
        # STOP-LOSS — OR logic inline por tick (v1.5.0 / v1.6.0)
        #
        ...
        #
        # v1.6.0 Anti-Pânico:
        #   SL_BASE_TRIGGER: 0.35 -> 0.25 (linha de perigo mais baixa)
        #   SL_PANIC_OBI:    0.05 -> 0.02 (colapso real = apenas 2% compradores)
        #
            ...
            for _sl_side, _sl_bid, _sl_z, _sl_obi, _sl_vpin in (
                ...
                if _sl_vpin is not None and _sl_vpin >= SL_TOXIC_VPIN:
                    ...
                    _sl_reason = (
                        ...
                        f"(colapso real confirmado — apenas {SL_PANIC_OBI:.0%} compradores)"
                    )
                else:
        ...
        # =====================================================================
        # TAKE-PROFIT DINAMICO — Wick Capture (v1.5.0 / v1.6.0)
        #
        # Se Z-Score(Kalman) >= TP_SPIKE_ZSCORE (3.0): o preco disparou de forma
        # estatisticamente absurda vs. a sua trajectoria Kalman dos ultimos 10s.
        ...
        #
        # Gap garantido: entrada Z <= 1.1, TP Z >= 3.0 (gap de 1.9 StdDev).
        ...
        # =====================================================================
        # MODULO 2: GAMBLING — Motor Quantitativo HFT (v1.5.0 / v1.6.0)
        #
        # 4 condições de entrada em simultâneo (AND logic):
        #
        #   Cond 1 — REGIME (σ): std(Kalman,10s) <= GAMB_MAX_VOL_DEV (0.04)
        ...
        #
        #   Cond 2 — Z-SCORE: Z(Kalman) <= _eff_zscore_limit
        #     Normal: GAMB_MAX_ZSCORE=1.1 | ENDGAME (<=30.999s): 99.0 (desativado)
        #     Z > limite = preco ja subiu acima da media — armadilha de topo.
        #     None (janela insuf.): bloqueia entrada.
        #
        ...
        #
        #   Cond 4 — VPIN (fluxo saudavel): VPIN <= _eff_vpin_limit
        #     Normal: VPIN_SAFE_LIMIT=0.40 | ENDGAME (<=30.999s): 0.60 (relaxado)
        ...
        #
        # eff_c maxima: GAMB_MAX_EFF_C=78.0c (v1.6.0: bloqueia acima de 78c).
        # =====================================================================
        if GAMBLING_ACTIVE:
            if rem > GAMB_START_REM_S:
                ...
                # Calculado UMA VEZ por tick, fora do loop de lados, para eficiência.
                # Globals GAMB_MAX_ZSCORE e VPIN_SAFE_LIMIT NUNCA são mutados.
                # _eff_zscore_limit e _eff_vpin_limit são variáveis locais por tick.
                if rem <= ENDGAME_TRIGGER_S:
                    _eff_zscore_limit = ENDGAME_ZSCORE_LIMIT   # 99.0 = Cond 2 desativada
                    _eff_vpin_limit   = ENDGAME_VPIN_LIMIT     # 0.60 = Cond 4 relaxada
                    _endgame_active   = True
                else:
                    _eff_zscore_limit = GAMB_MAX_ZSCORE         # valor global normal (1.1)
                    _eff_vpin_limit   = VPIN_SAFE_LIMIT          # valor global normal (0.40)
                ...
                for g_side, g_ask, g_bid, g_eff, g_z, g_std, g_obi, g_vpin in (
                    ...
                    # Limite efectivo: ENDGAME_ZSCORE_LIMIT(99.0) em modo ENDGAME
                    #                  (desativado — nunca bloqueará),
                    #                  ou GAMB_MAX_ZSCORE(1.1) em modo normal.
                    if g_z is None:
                    ...
                    # Nao alterada pelo ENDGAME override (Kalman e OBI intactos).
                    ...
                    # Limite efectivo: ENDGAME_VPIN_LIMIT(0.60) em modo ENDGAME
                    #                  (relaxado vs. VPIN_SAFE_LIMIT),
                    #                  ou VPIN_SAFE_LIMIT(0.40) em modo normal.
                    _mode_tag_vpin = (
                    ...
                    if g_vpin is not None and g_vpin > _eff_vpin_limit:
                        log_m("GAMBLING", "BLOCK",
                            ...
                            f"(fluxo toxico — sem condicoes de entrada) {_mode_tag_vpin}")
                        continue
                    elif g_vpin is None:
                    ...
                    _endgame_entry_tag = (
                        f" | ⚡ ENDGAME MODE "
                        f"(Z_lim={ENDGAME_ZSCORE_LIMIT:.0f} "
                        f"VPIN_lim={ENDGAME_VPIN_LIMIT:.2f})"
                        if _endgame_active else ""
                    )
                    if bankroll > 0.0:
...
async def main():
    ...
    # ─ Log de arranque ───────────────────────────────────────────────────────
    log_sep2()
    log_info("BOT XRP POLYMARKET v1.6.0 INICIADO — 6 REFACTORINGS CIRURGICOS (WIN RATE 95%)")
    log_sep2()
    ...
    log_info(f"   HFT Window      : {HFT_WINDOW_SECONDS}s (memoria do mercado — ultra-reativo)")
    log_info(f"   VPIN Window     : {HFT_WINDOW_SECONDS}s (classificacao fluxo)")
    log_info(f"   GAMB_MAX_VOL_DEV: {GAMB_MAX_VOL_DEV:.3f} (squeeze apertado — v1.6.0: 0.10->0.04)")
    log_sep2()
    log_info("MICRO-TREND (30s micro-ciclos — v1.6.0):")
    log_info(f"   Window          : {MICRO_TREND_WINDOW_S:.0f}s (rolling — acumula Kalman)")
    log_info(f"   Threshold       : {MICRO_TREND_THRESHOLD:.3f} (Δ mínimo UP/DOWN)")
    log_info(f"   Log interval    : {MICRO_TREND_LOG_INTERVAL_S:.0f}s por ciclo")
    log_info(f"   Função          : diagnóstico — não bloqueia entradas Gambling")
    log_sep2()
    log_info("PEG ARBIT:")
    ...
    log_sep2()
    log_info("GAMBLING (4 condições HFT simultâneas — v1.6.0):")
    log_info(f"   Activo          : {'ON' if GAMBLING_ACTIVE else 'OFF'}")
    log_info(f"   Cond 1 Regime   : σ(Kalman,{HFT_WINDOW_SECONDS}s) <= {GAMB_MAX_VOL_DEV:.3f} (squeeze; v1.6.0: 0.10->0.04)")
    log_info(f"   Cond 2 Z-Score  : Z(Kalman)     <= {GAMB_MAX_ZSCORE:.1f} (anti-topo; v1.6.0: 1.8->1.1)")
    log_info(f"   Cond 3 OBI      : OBI           >= {GAMB_MIN_OBI:.0%} (suporte real)")
    log_info(f"   Cond 4 VPIN     : VPIN          <= {VPIN_SAFE_LIMIT:.0%} (fluxo OK; v1.6.0: 55%->40%)")
    log_info(f"   eff_c maxima    : {GAMB_MAX_EFF_C:.0f}c (v1.6.0: 95c->78c — gap de lucro)")
    log_info(f"   Gestao risco    : Kelly (edge={KELLY_ASSUMED_EDGE:.0%} frac=1/{int(round(1/KELLY_FRACTION))} cap={KELLY_MAX_RISK_PCT:.0%})")
    log_info(f"   Martingale      : REMOVIDO — Kelly substitui")
    log_sep2()
    log_info("ENDGAME OVERRIDE (Últimos 30.999s — v1.6.0):")
    log_info(f"   Trigger         : remaining <= {ENDGAME_TRIGGER_S:.3f}s")
    log_info(f"   Cond 2 Z-Score  : {GAMB_MAX_ZSCORE:.1f} -> {ENDGAME_ZSCORE_LIMIT:.0f} (desativado — aceita picos finais)")
    log_info(f"   Cond 4 VPIN     : {VPIN_SAFE_LIMIT:.2f} -> {ENDGAME_VPIN_LIMIT:.2f} (relaxado — aceita toxicidade de fecho)")
    log_info(f"   Globals         : GAMB_MAX_ZSCORE e VPIN_SAFE_LIMIT nunca mutados")
    log_sep2()
    log_info("TAKE-PROFIT DINÂMICO (Wick Capture — v1.5.0/v1.6.0):")
    ...
    log_info(f"   Gap entrada/TP  : entrada Z<={GAMB_MAX_ZSCORE:.1f} vs TP Z>={TP_SPIKE_ZSCORE:.1f} (gap={TP_SPIKE_ZSCORE-GAMB_MAX_ZSCORE:.1f} StdDev)")
    log_info(f"   Logica          : Z(Kalman) >= {TP_SPIKE_ZSCORE} => sell imediato ao BID")
    log_sep2()
    log_info("STOP-LOSS ANTI-PÂNICO (OR logic — v1.5.0/v1.6.0):")
    log_info(f"   Activo          : {'ON' if STOP_LOSS_ACTIVE else 'OFF'}")
    log_info(f"   Base trigger    : BID <= {SL_BASE_TRIGGER:.2f} ({SL_BASE_TRIGGER*100:.0f}c; v1.6.0: 35c->25c)")
    ...
    log_info(f"   Trigger C OBI   : OBI <= {SL_PANIC_OBI:.2f} ({SL_PANIC_OBI*100:.0f}% compradores — colapso real; v1.6.0: 5%->2%)")
    ...
    while True:
        ...
        if best_bids["up"] is not None:
            ...
            # v1.5.0/v1.6.0: logic_loop sem parametros Martingale
