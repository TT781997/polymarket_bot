# =============================================================================
# BOT XRP POLYMARKET — v1.9.0
# =============================================================================
# CHANGELOG v1.9.0 [Aggressive Endgame + FOK/FAK + Background Resolution Poller]:
#
# ─────────────────────────────────────────────────────────────────────────────
# ARQUITECTURA v1.9.0 — 3 NOVOS MÓDULOS INTEGRADOS (bot_modules_v1_9.py)
# ─────────────────────────────────────────────────────────────────────────────
#
# [1] REST RESOLUTION POLLER (background — não bloqueia o loop):
#     Ao expirar o mercado, o bot já NÃO espera 600s pela resolução WS.
#     Em vez disso: lança ResolutionPoller em background (REST poll cada 20s),
#     cancela o WS, e avança IMEDIATAMENTE para o mercado seguinte.
#     O poller resolve PnL via callback async quando a Gamma API confirma.
#     Resultado: zero downtime entre mercados (antes: até 2 min bloqueado).
#     Parâmetros: POLL_INTERVAL_S=20, POLL_MAX_DURATION_S=600.
#
# [2] AGGRESSIVE ENDGAME (últimos 5 segundos — bypassa ALL HFT filters):
#     Nos 5s finais, liquidez seca e spreads explodem — os filtros HFT
#     rejeitam TUDO. Novo módulo: se ASK de UP ou DOWN está entre 75c-95c,
#     compra imediatamente via FAK (grab all liquidity).
#     Ignora: spread, σ, Z-score, OBI, VPIN, Kelly, BID/ASK ratio.
#     Parâmetros: AGGRESSIVE_ENDGAME_ACTIVE, AGGRESSIVE_ENDGAME_RISK=10%.
#
# [3] FOK/FAK ORDER EXECUTION (substituição do GTC limit):
#     place_live_order() agora usa FOK (Fill-Or-Kill) com slippage protection.
#     Endgame usa FAK (Fill-And-Kill) para máxima agressividade.
#     Slippage: BUY worst_price = ask + SLIPPAGE_TOLERANCE (2c).
#               SELL worst_price = bid - SLIPPAGE_TOLERANCE.
#     Code path IDÊNTICO em DEMO e LIVE (mesmo log, mesmo flow).
#     Parâmetros: SLIPPAGE_TOLERANCE=0.02.
#
# FUNÇÕES/CLASSES AFECTADAS v1.9.0:
#   - Imports: bot_modules_v1_9 (ResolutionPoller, FOK/FAK, Endgame)
#   - Parâmetros: SLIPPAGE_TOLERANCE, AGGRESSIVE_ENDGAME_*, POLL_*
#   - SDK: OrderType + SELL importados (para FOK/FAK)
#   - place_live_order(): GTC → FOK com slippage (via bot_modules_v1_9)
#   - logic_loop(): novo bloco AGGRESSIVE ENDGAME (rem<=5s, antes de GAMBLING)
#   - logic_loop(): resolução non-blocking (trades pendentes + poller background)
#   - main(): gestão de pollers em background; cancel em shutdown
#
# =============================================================================
...
# v1.9.0: módulos externos (ResolutionPoller, FOK/FAK, Endgame)
from bot_modules_v1_9 import (
    # Module 1 — Resolution Poller
    ResolutionPoller,
    ResolutionResult,
    PollerState,
    # Module 2 — Aggressive Endgame
    evaluate_aggressive_endgame,
    EndgameSignal,
    # Module 3 — FOK/FAK Execution
    execute_fok_order,
    execute_fak_order,
    OrderResult,
    # Slippage helper
    _compute_worst_price,
)
...
# --- SLIPPAGE PROTECTION (novo v1.9.0) ---
...
# --- AGGRESSIVE ENDGAME (novo v1.9.0 — últimos 5 segundos) ---
#
...
AGGRESSIVE_ENDGAME_RISK   = 0.10   # Fracção da banca por trade endgame (10%) [Range: 0.05 | 0.30]
...
# --- RESOLUTION POLLER (novo v1.9.0 — background REST polling) ---
#
#   Quando o mercado expira, em vez de bloquear à espera de market_resolved WS,
#   o bot lança um poller REST em background e avança para o mercado seguinte.
#   O poller poll'a a Gamma API cada 20s e dispara callback de PnL ao resolver.
#
POLL_INTERVAL_S       = 20.0   # Intervalo entre polls REST (seg) [Range: 5.0 | 60.0]
POLL_MAX_DURATION_S   = 600.0  # Duração máxima do poller (seg) [Range: 120.0 | 1200.0]
POLL_REQUEST_TIMEOUT_S = 10.0  # Timeout por pedido REST (seg) [Range: 5.0 | 30.0]
...
RESOLVE_MAX_WAIT_S = 600.0  # Safety net: tempo maximo a aguardar market_resolved WS (seg) [Range: 120.0 | 1200.0]
                             # v1.8.0: se timeout, REFUND total_out (PnL NAO registado).
                             # Não estima vencedor por BID — exige resolução oficial.
...
# =============================================================================
# LIVE ORDER (v1.9.0 — FOK com slippage via bot_modules_v1_9)
# =============================================================================

async def place_live_order(side: str, ask: float, shares: float, token_id: str) -> bool:
    """
    Ordem FOK BUY ao ASK via py_clob_client (v1.9.0).

    v1.9.0: Substituiu GTC limit por FOK (Fill-Or-Kill) com slippage protection.
            worst_price = ask + SLIPPAGE_TOLERANCE (garante fill em mercados rápidos).
            Protegido por circuit_breaker — não tenta se API em falha.
            Code path IDÊNTICO em DEMO e LIVE (via bot_modules_v1_9).
    """
    if api_circuit_breaker.is_open():
    ...
    # BUY: amount = dollar amount (invested_pure = shares * ask)
...
# =============================================================================
# LOGIC LOOP (v1.8.0 — real-time settlement, sem fallback BID)
# =============================================================================

async def logic_loop(m_start: float, m_end: float, meta: dict) -> list:
    """
    Loop principal de trading para um ciclo de 5 minutos.

    v1.9.0: Retorna lista de trades pendentes (não resolvidos) ao fim
    do mercado. O caller (main) lança ResolutionPoller em background
    para resolver PnL sem bloquear.
    ...
    v1.8.0 — REAL-TIME SETTLEMENT:
               Fim de mercado: await resolved_event.wait() (sem timeout de BID).
               RESOLVE_MAX_WAIT_S = 600s como safety net.
               Se timeout: REFUND total_out; PnL NÃO registado.
               Estimativa por BID REMOVIDA — exige resolução oficial.
    """
    ...
    log_info(
        f"v1.8.0: Real-Time Settlement | "
        f"RESOLVE_MAX_WAIT_S={RESOLVE_MAX_WAIT_S:.0f}s | "
        f"sem fallback BID"
    )
    log_sep()
    ...
    # v1.8.0: SÓ chamado após market_resolved WS (nunca por estimativa BID)
    # =========================================================================
    ...
    while not _shutdown_flag:
        ...
        # ── Fim de mercado (v1.8.0 — real-time settlement) ────────────────
        #
        # ARQUITECTURA v1.8.0:
        #   1. rem <= 0: mercado expirado.
        #   2. Se existem trades activos: aguardar market_resolved WS.
        #   3. SÓ close_trade_resolution() após evento oficial.
        #   4. Se timeout (RESOLVE_MAX_WAIT_S): REFUND, PnL NÃO registado.
        #   5. REMOVIDO: estimativa de vencedor por BID final.
        #   6. REMOVIDO: RESOLVE_TIMEOUT_S = 35s (era demasiado curto).
        #
        if rem <= 0.0:
            ...
            if active_trades:
                # v1.9.0: NÃO bloqueia — retorna trades pendentes para o caller.
                # O main() lança ResolutionPoller em background para resolver PnL.
                log_info(
                    f"v1.9.0: {len(active_trades)} trade(s) pendente(s) — "
                    f"resolucao delegada ao ResolutionPoller em background"
                )
        ...
        # =====================================================================
        # MODULO 0: AGGRESSIVE ENDGAME (v1.9.0 — últimos 5 segundos)
        #
        # BYPASSA TODOS os filtros HFT standard. Nos últimos 5s o mercado
        # converge para o resultado — se um lado tem ASK entre 75c-95c,
        # é muito provavelmente o vencedor. Compra imediata via FAK.
        #
        # Ignora: MAX_SPREAD_CENTS, GAMB_MAX_VOL_DEV, GAMB_MAX_ZSCORE,
        #         GAMB_MIN_OBI, VPIN_SAFE_LIMIT, BID_ASK_MIN_RATIO,
        #         GAMB_MIN_EFF_C, GAMB_MAX_EFF_C, Kelly Criterion.
        #
        # Só dispara UMA VEZ (flag endgame_fired) para evitar entradas
        # em cascata nos últimos 5 ticks.
        # =====================================================================
        if (AGGRESSIVE_ENDGAME_ACTIVE
                and 0.0 < rem <= AGGRESSIVE_ENDGAME_S
                and not endgame_fired
                and bankroll > 0.0):
            _eg_signal = evaluate_aggressive_endgame(
                remaining_s=rem,
                best_asks=best_asks,
                meta=meta,
                trigger_s=AGGRESSIVE_ENDGAME_S,
                min_price=AGGRESSIVE_ENDGAME_MIN_C,
                max_price=AGGRESSIVE_ENDGAME_MAX_C,
            )
            if _eg_signal is not None:
                endgame_fired = True
                _eg_invest = bankroll * AGGRESSIVE_ENDGAME_RISK
                _eg_ask    = _eg_signal.ask_price
                _eg_side   = _eg_signal.side
                _eg_tid    = _eg_signal.token_id

                log_sep()
                log_m("ENDGAME_AGG", "FIRE",
                    f"rem={rstr} | {_eg_side} @ ASK={fc(_eg_ask)} | "
                    f"invest=${_eg_invest:.4f} ({AGGRESSIVE_ENDGAME_RISK:.0%}) | "
                    f"conf={_eg_signal.confidence:.2f} | "
                    f"ALL HFT FILTERS BYPASSED | FAK+SLIP={SLIPPAGE_TOLERANCE}")

                # Executar FAK (grab all liquidity)
                _eg_result = await execute_fak_order(
                    client=clob_client,
                    token_id=_eg_tid,
                    side="BUY",
                    amount=_eg_invest,
                ...
                if _eg_result.success:
                    # Registar trade em active_trades
                    ...
                    log_m("ENDGAME_AGG", "BUY",
                        f"rem={rstr} | {_eg_side} @ ASK={fc(_eg_ask)} | "
                        f"shares={_eg_shares:.4f} | total=${_eg_total_out:.4f} | "
                        f"orderID={_eg_result.order_id}")
                else:
                    log_warn(
                        f"ENDGAME_AGG FAK falhou: {_eg_result.error} "
                        f"(sem liquidez — mercado já fechou?)")
                log_sep()
...
# =============================================================================
# MAIN (v1.9.0 — background resolution pollers + aggressive endgame)
# =============================================================================
...
async def main():
    ...
    # v1.9.0: lista de pollers em background
    active_pollers: list = []

    # ─ Log de arranque ───────────────────────────────────────────────────────
    log_sep2()
    log_info("BOT XRP POLYMARKET v1.9.0 — AGGRESSIVE ENDGAME + FOK/FAK + BACKGROUND POLLER")
    log_sep2()
    log_info(f"LIVE_TRADING     : {LIVE_TRADING} (reverter: LIVE_TRADING = False)")
    log_info(f"BANKROLL_INIT    : ${bankroll:.2f}")
    log_sep2()
    log_info("v1.9.0 — NOVOS MÓDULOS:")
    log_info(f"   Aggressive Endgame : active={AGGRESSIVE_ENDGAME_ACTIVE} | risk={AGGRESSIVE_ENDGAME_RISK:.0%} | trigger<={AGGRESSIVE_ENDGAME_S}s | range=[{AGGRESSIVE_ENDGAME_MIN_C*100:.0f}c-{AGGRESSIVE_ENDGAME_MAX_C*100:.0f}c]")
    log_info(f"   FOK/FAK Orders     : slippage={SLIPPAGE_TOLERANCE} (BUY: +{SLIPPAGE_TOLERANCE*100:.0f}c / SELL: -{SLIPPAGE_TOLERANCE*100:.0f}c)")
    log_info(f"   Resolution Poller  : interval={POLL_INTERVAL_S}s | max={POLL_MAX_DURATION_S:.0f}s | timeout={POLL_REQUEST_TIMEOUT_S}s")
    log_sep2()
    ...
    while not _shutdown_flag:
        ...
        if best_bids["up"] is not None:
            ...
            # v1.9.0: logic_loop retorna trades pendentes (não resolvidos)
            pending_trades = await logic_loop(start_ts, start_ts + 300, meta)
            ...
            if pending_trades:
                log_info(
                    f"PENDING | {len(pending_trades)} trade(s) aguardando resolucao "
                    f"via poller background"
                )
            log_sep2()
        else:
            log_warn("Sem BIDs/ASKs recebidos neste ciclo — a saltar")

        # ── v1.9.0: Matar WS e lançar poller em background ──────────────────
        ws_task.cancel()
        try:
            await ws_task
        except asyncio.CancelledError:
        ...
        # Se existem trades pendentes, lançar poller para resolver PnL em background
        if pending_trades:
            _current_meta = meta
            _current_pending = list(pending_trades)

            def _make_resolution_callback(trades_to_settle, market_meta):
                """Factory para criar callback com closure sobre os trades pendentes."""
                async def _on_resolution(result: ResolutionResult) -> None:
                    global bankroll, total_pnl_pos, total_pnl_neg
                    if result.resolved and result.winner_token_id:
                        log_sep()
                        log_info(
                            f"POLLER CALLBACK | Mercado resolvido: "
                            f"{result.winning_outcome} | "
                            f"token={result.winner_token_id[:16]}... | "
                            f"slug={market_meta['slug']} | "
                            f"polls={result.polls_made} | "
                            f"elapsed={result.elapsed_s:.1f}s"
                        )
                        for trade in trades_to_settle:
                            _winner = (trade.get("token_id") == result.winner_token_id)
                            shares  = trade["shares"]
                            payout  = shares if _winner else 0.0
                            pnl     = payout - trade["total_out"]
                            bankroll += payout
                            if pnl > 0.00001:
                                total_pnl_pos += pnl
                            elif pnl < -0.00001:
                                total_pnl_neg += pnl
                            _reason = "RESOLUCAO GANHA ($1/share)" if _winner else "RESOLUCAO PERDIDA (Total)"
                            _price  = "100.0c" if _winner else "0.0c"
                            _sign   = "(+)" if pnl >= 0 else "(-)"
                            if LIVE_TRADING and _winner and trade.get("token_id"):
                                redeem_live_position(shares, trade["token_id"])
                            log_m(trade["type"], "SELL",
                                f"rem=00:00:000 | {trade['side']} @ {_price} "
                                f"| net=${payout:.4f} "
                                f"| PnL: ${pnl:+.4f} ({pnl/trade['total_out']*100 if trade['total_out'] else 0:+.2f}%) {_sign} "
                                f"| Reason: {_reason} [POLLER]")
                        log_sep()
                    else:
                        log_warn(
                            f"POLLER CALLBACK | Sem resolucao para "
                            f"{market_meta['slug']} apos "
                            f"{result.elapsed_s:.0f}s ({result.polls_made} polls) — "
                            f"REFUND de {len(trades_to_settle)} trade(s)"
                        )
                        for trade in trades_to_settle:
                            bankroll += trade["total_out"]
                            log_warn(
                                f"REFUND | {trade['side']} {trade['type']} "
                                f"| total_out=${trade['total_out']:.4f} devolvido a banca "
                                f"| sem resolucao oficial — PnL nao registado [POLLER]"
                            )
                return _on_resolution

            _callback = _make_resolution_callback(_current_pending, _current_meta)

            poller = ResolutionPoller(
                slug=_current_meta["slug"],
                condition_id=_current_meta.get("id", ""),
                token_up=_current_meta["up"],
                token_down=_current_meta["down"],
                gamma_api_url=GAMMA_API_URL,
                on_resolved=_callback,
                poll_interval_s=POLL_INTERVAL_S,
                max_duration_s=POLL_MAX_DURATION_S,
                request_timeout_s=POLL_REQUEST_TIMEOUT_S,
            )
            _poller_task = poller.start()
            active_pollers.append((poller, _poller_task))
            log_info(
                f"POLLER LAUNCHED | slug={_current_meta['slug']} | "
                f"{len(_current_pending)} trade(s) pendente(s) | "
                f"interval={POLL_INTERVAL_S}s | max={POLL_MAX_DURATION_S:.0f}s"
            )

        # Limpar pollers terminados
        active_pollers = [
            (p, t) for p, t in active_pollers if not t.done()
        ]
        if active_pollers:
            log_info(f"POLLERS ACTIVOS: {len(active_pollers)} em background")

        # v1.9.0: avança IMEDIATAMENTE para o mercado seguinte
        await asyncio.sleep(0.5)

    # ── Shutdown gracioso ─────────────────────────────────────────────────────
    # Cancelar pollers pendentes
    for poller, task in active_pollers:
        poller.cancel()
    for _, task in active_pollers:
        try:
            await task
