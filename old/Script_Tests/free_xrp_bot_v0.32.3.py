...
EIGHTY_MAX_EFF_C          = 99.5  # cents 0.0–100.0  (85.0 ... 99.5)   — cap a 94c; acima disso retorno <6.4% não justifica o risco de perder
EIGHTY_MIN_TICKS          = 4     # int    (1 ... 20)                   — 6 níveis confirmados (era 5) — mais confirmação de consolidação
EIGHTY_RISK               = 0.09  # rácio 0.0–1.0  (0.01 ... 0.15)     — 9% da banca por trade; equilibra retorno vs perda máxima aceitável
EIGHTY_CUTOFF_S           = 0   # segundos (0.0 ... 60.0)  — para a 8s do fim (era 5s); janela activa: 45s→8s
EIGHTY_WHEN_CUTOFF_0_VOLT = 5  # segundos (0.0 ... 60.0)  — só activo se CUTOFF_S=0: ignora vol nos últimos N segundos
EIGHTY_PEG_MIN_C          = 98.0  # cents 0.0–200.0  (90.0 ... 102.0)  — PEG mínimo 99c (era 98c); mercado muito equilibrado = maior convicção no lado vencedor
EIGHTY_BUY_COOLDOWN       =  3.0  # segundos (0.0 ... 30.0)  — 5s entre compras (era 3s); limita sobre-exposição no mesmo round
...
EIGHTY_VOL_COOLDOWN_S     =  4.0  # segundos (0.0 ... 30.0)  — cooldown mais longo após vol (era 5s); protege contra entradas rápidas pós-spike

# [v0.30.4] Delta Multi-Timeframe
EIGHTY_DELTA_INTERVALS    = [0.5, 1.0, 2.0]  # segundos        — intervalos de comparação de delta de preço
EIGHTY_DELTA_LOOKBACK_S   =  1,5  # segundos (0.5 ... 10.0)  — lookback máximo do buffer de delta
EIGHTY_DELTA_MAX_RISE_C   =  4.0  # cents 0.0–100.0  (2.0 ... 15.0)   — era 5.0c; sobe mais rápido que isto = suspeita de pump falso
EIGHTY_DELTA_VOL_RISE_C   =  5.0  # cents 0.0–100.0  (2.0 ... 15.0)   — era 4.5c; mais sensível a subidas rápidas que sinalizam reversão
...
PEG_ARBIT_UNDERPEG_C    = 1.0     # cents 0.0–200.0  (2.0 ... 20.0)   — era 5.0c; baixar para 3.0c activa com mais frequência (PEG < 0.97)
PEG_ARBIT_RISK          = 0.20    # rácio 0.0–1.0  (0.05 ... 0.30)    — era 0.15; subir para 0.18 aumenta lucro por arb (~+20% vs anterior)
...
MAX_PEG_ENTRIES         = 100000  # int    (1 ... 50)                   — era 10; subir para 15 captura mais oportunidades por round
TARGET_MULTIPLIER       = 1.25    # multiplicador ≥ 1.0  (1.02 ... 1.20)    — multiplicador sobre eff para definir target do PEG_ARBIT
...
async def logic_loop(m_start: float, m_end: float, meta: dict, r_mult: float):
    ...
    async def open_trade(
        ...
        if trade_type.startswith('CICLO'):
            ...
            target = min(0.99, PEG_ARBIT_TARGET_C / 100.0) if PEG_ARBIT_TARGET_C > 0 else None
        else:
