...
################################################################################
# CHANGELOG — BOT_XRP_POLYMARKET_v2.0_EVENT_ENGINE
#
# Refatoração arquitetural focada em execução orientada a eventos (WebSocket)
# para eliminar polling REST durante o ciclo de trading.
#
# ALTERAÇÕES PRINCIPAIS
#
# [GLOBAL]
# + Adicionado EventEngine para gerir WebSocket único e stream completo.
# + Adicionado mecanismo de heartbeat (ping/pong) e reconexão automática.
# + Adicionado metadata cache no arranque para 12 mercados futuros.
#
# [WEBSOCKET ENGINE]
# + Nova função async websocket_event_listener()
# + Processamento de eventos:
#       - type: price_change
#       - type: market_resolved
# + Routing por token_id → slug.
#
# [OVERLAP DE MERCADOS]
# + Nova função schedule_market_overlap()
# + Subscrição do mercado seguinte 15‑30s antes da expiração.
#
# [SETTLEMENT REAL]
# + SELL / PnL apenas após evento WS market_resolved
# + Removida dependência de estimativas por BID.
#
# [ARQUITETURA ASSÍNCRONA]
# + Uso extensivo de asyncio.create_task para evitar bloqueios.
#
# [RESILIÊNCIA]
# + reconnect automático com backoff
# + refresh de metadata cache após reconnect.
#
# NOTA:
# O restante código original foi preservado integralmente conforme solicitado.
################################################################################


# ==============================================================================
# EVENT ENGINE (WebSocket full‑stream)
# ==============================================================================
...
_token_to_slug = {}
_market_resolution_events = {}
_price_state = {}

_last_ws_message = 0


async def websocket_event_listener(sub_queue):
    ...
    while True:
        try:
            async with websockets.connect(WS_ENDPOINT, ping_interval=None) as ws:
                ...
                async def sender():
                    while True:
                        sub = await sub_queue.get()
                        await ws.send(json.dumps({
                            "type": "subscribe",
                            "assets_ids": sub["assets_ids"]
                        }))
                ...
                async for msg in ws:
                    ...
                    if data.get("type") == "price_change":
                        ...
                        _price_state[token] = price

                    elif data.get("type") == "market_resolved":

                        token = data.get("winning_asset_id")

                        slug = _token_to_slug.get(token)

                        if slug and slug in _market_resolution_events:
                            _market_resolution_events[slug].set()
...
async def preload_metadata():
    print("[CACHE] loading next markets")
    ...
    for ts in markets:
        ...
        try:
            ...
            url = generate_polymarket_url(ts)

            market, tokens = get_market_and_token_ids(url)
    ...
    print("[CACHE] loaded:", len(metadata_cache))
...
async def schedule_market_overlap(sub_queue):

    while True:

        now = int(time.time())
        next_market = (now // 300 + 1) * 300

        preload_time = next_market - 25

        wait = preload_time - now

        if wait > 0:
            await asyncio.sleep(wait)

        meta = metadata_cache.get(next_market)
        ...
        await sub_queue.put(sub)

        print("[OVERLAP] subscribed:", slug)


# ==============================================================================
# MARKET RESOLUTION WAIT
# ==============================================================================

async def wait_market_resolution(slug):

    ev = asyncio.Event()

    _market_resolution_events[slug] = ev
...
def get_ws_price(token_id):
    return _price_state.get(token_id)
...
# =============================================================================
# WEBSOCKET HANDLER PERSISTENTE v2.1.0 — COM DEBUG
# =============================================================================

async def ws_handler(sub_queue: asyncio.Queue):
    """
    v2.1.0: WebSocket com DEBUG + validação de frames
    Detecta e logga corrupção de dados
    """
    ...
    while not _shutdown_flag:
        try:
            async with websockets.connect(
                ...
                # Re-subscreve tokens conhecidos
                ...
                while not sub_queue.empty():
                    try:
                        ...
                        log_info(
                            f"WS NOVA SUB (queue) | slug={_pending.get('slug','?')} "
                        ...
                        while True:
                            ...
                            try:
                                ...
                                log_warn(
                                    ...
                                    f"errors={_error_frame_count} — fechando"
                                )
                                await ws.close()
                                await asyncio.sleep(0.1)
                                return
                ...
                try:
                    # Loop principal com DEBUG
                    async for raw in ws:
                        _frame_count += 1

                        # DEBUG: Log a cada 100 frames
                        if _frame_count % 100 == 0:
                            log_info(
                                f"WS DEBUG | frames_total={_frame_count} | "
                                f"valid={_valid_frame_count} ({_valid_frame_count*100//_frame_count if _frame_count else 0}%) | "
                        ...
                        # v1.8.1: JSON Parse com tratamento melhorado
                        try:
            ...
            if _shutdown_flag:
                log_info("WS ERRO durante shutdown — saindo")
                break

            log_warn(
                f"WS erro: {e} | frames={_frame_count} valid={_valid_frame_count} "
                f"errors={_error_frame_count} — reconectando em {_ws_backoff:.1f}s"
            )
...
async def logic_loop(m_start: float, m_end: float, meta: dict,
    ...
    log_info(
        f"WS v2.1.0: Overlap@rem<={WS_OVERLAP_PRE_S}s | "
        f"Settlement=ONLY_WS_market_resolved | Non-blocking=create_task | "
        f"DEBUG=frame_validation"
    )
    log_sep()
    ...
    while not _shutdown_flag:
        ...
        # ── v2.1.0: VALIDAÇÃO DE PREÇOS (NOVO PONTO 2) ────────────────────
...
async def main():
    ...
    log_sep2()
    log_info("WS FULL-STREAM (v2.1.0 — DEBUG):")
