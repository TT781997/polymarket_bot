...
GAMB_MIN_OBI       = 0.75   # OBI >= 75%: compradores dominam o Top of Book; v1.7.0: 0.70->0.75 [Range: 0.50 | 0.90]
...
#
KELLY_ASSUMED_EDGE = 0.05   # Vantagem probabilistica assumida quando HFT da luz verde (+5%) [Range: 0.01 | 0.20]
KELLY_FRACTION     = 0.08   # Fractional Kelly: ~1/12 de Kelly para seguranca maxima [Range: 0.05 | 1.0]
KELLY_MAX_RISK_PCT = 0.05   # Cap absoluto de risco por trade Kelly (5% da banca) [Range: 0.01 | 0.25]
...
def calc_imbalance(bid_size, ask_size):
    ...
    Interpretacao:
      >= GAMB_MIN_OBI (0.75): compradores dominam — suporte real.
...
# =============================================================================
# WEBSOCKET HANDLER (com backoff exponencial em reconnect — v1.7.0)
# =============================================================================
...
async def ws_handler(t_up: str, t_down: str):
    ...
    v1.7.0: backoff exponencial em reconnect (1s→2s→4s→8s→16s máx).
            ...
            v1.4.0/v1.6.0 usava sleep(1) fixo em qualquer erro WS.
    ...
    while not _shutdown_flag:
        try:
            async with websockets.connect(WS_URI, ping_interval=20, ping_timeout=10) as ws:
                ...
                log_info(f"WS conectado ao orderbook Polymarket (backoff_reset={WS_RECONNECT_BASE_S}s)")
                ...
                async for raw in ws:
                    ...
                    for item in items:
                        ...
                        if evt == "book":
                            ...
                            try:
                                sp_c = await fetch_spread_sdk(aid)
                                if sp_c is not None:
                                    _sprc[sk] = sp_c
                            except Exception:
                                pass
...
async def logic_loop(m_start: float, m_end: float, meta: dict):
    ...
    while not _shutdown_flag:
        ...
        #   Cond 3 — OBI: OBI >= GAMB_MIN_OBI (0.75)
...
# =============================================================================
# MAIN (com graceful shutdown SIGTERM — v1.7.0)
# =============================================================================
...
async def main():
    ...
    # ─ Log de arranque ───────────────────────────────────────────────────────
    log_sep2()
    log_info("BOT XRP POLYMARKET v1.7.1 — PRODUCTION READY + WIN RATE 95% (SEM TREND)")
    log_sep2()
    ...
    log_info(f"   Kalman Q        : {KALMAN_PROCESS_NOISE:.0e}")
    log_info(f"   Kalman R        : {KALMAN_MEASURE_NOISE:.0e}")
    log_info(f"   HFT Window      : {HFT_WINDOW_SECONDS}s")
    log_info(f"   VPIN Window     : {HFT_WINDOW_SECONDS}s")
    log_info(f"   GAMB_MAX_VOL_DEV: {GAMB_MAX_VOL_DEV:.3f} (v1.7.0: 0.04->0.03)")
    log_sep2()
    log_info("TAKE-PROFIT v1.7.0 — FIX CRÍTICO (portão duplo de lucratividade):")
    log_info(f"   TP_SPIKE_ZSCORE : {TP_SPIKE_ZSCORE:.1f} (v1.6.0 era 3.0 — demasiado sensivel)")
    log_info(f"   Portao 1        : BID >= ASK_entrada + {TP_MIN_BID_OVER_ASK*100:.0f}c (break-even minimo)")
    log_info(f"   Portao 2        : sell_payout_net > total_out * {1+TP_MIN_PROFIT_PCT:.3f} ({TP_MIN_PROFIT_PCT:.1%} lucro minimo)")
    log_info(f"   Logica          : portao1 AND portao2 AND Z>={TP_SPIKE_ZSCORE}")
    log_info(f"   Antes (v1.6.0)  : Z>=3.0 => venda IMEDIATA (causava loss -7% e -1%)")
    log_sep2()
    log_info("GAMBLING (4 condições + pré-filtro BID/ASK — v1.7.0):")
    log_info(f"   Pre-filtro      : BID/ASK >= {BID_ASK_MIN_RATIO:.0%} (spread saudavel)")
    log_info(f"   GAMB_MIN_EFF_C  : {GAMB_MIN_EFF_C:.0f}c (v1.7.0: 65->67 — break-even > 69c)")
    log_info(f"   GAMB_MAX_ZSCORE : {GAMB_MAX_ZSCORE:.1f} (v1.7.0: 1.1->1.3)")
    log_info(f"   GAMB_MIN_OBI    : {GAMB_MIN_OBI:.0%} (v1.7.0: 70%->75%)")
    log_info(f"   GAMB_BUY_COOLDOWN: {GAMB_BUY_COOLDOWN:.0f}s (v1.7.0: 8->12)")
    log_info(f"   Gestao risco    : Kelly (edge={KELLY_ASSUMED_EDGE:.0%} frac=1/{int(round(1/KELLY_FRACTION))} cap={KELLY_MAX_RISK_PCT:.0%})")
    log_sep2()
    log_info("STOP-LOSS anti-pânico (OR logic — v1.7.0):")
    log_info(f"   Base trigger    : BID <= {SL_BASE_TRIGGER:.2f}")
    log_info(f"   Trigger A VPIN  : VPIN >= {SL_TOXIC_VPIN:.2f} (v1.7.0: 0.95->0.97)")
    log_info(f"   Trigger B Z     : Z   <= {SL_CRASH_ZSCORE:.1f} (v1.7.0: -4.0->-5.0)")
    log_info(f"   Trigger C OBI   : OBI <= {SL_PANIC_OBI:.2f} ({SL_PANIC_OBI*100:.0f}% compradores)")
    log_sep2()
    log_info("ENDGAME OVERRIDE (Últimos 30.999s):")
    log_info(f"   Cond 2 Z-Score  : {GAMB_MAX_ZSCORE:.1f} -> {ENDGAME_ZSCORE_LIMIT:.0f} (desativado)")
    log_info(f"   Cond 4 VPIN     : {VPIN_SAFE_LIMIT:.2f} -> {ENDGAME_VPIN_LIMIT:.2f} (relaxado)")
    log_sep2()
    ...
    while not _shutdown_flag:
        ...
        if best_bids["up"] is not None:
            ...
            # v1.7.0: logic_loop sem parametros Martingale
