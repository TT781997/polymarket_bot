...
SLIPPAGE_TOLERANCE = 0.04  # Tolerância de slippage em preço (0.02 = 2 cents) [Range: 0.01 | 0.05]
...
AGGRESSIVE_ENDGAME_S      = 2.0    # Segundos finais para activar [Range: 2.0 | 10.0]
...
#   7%: Lucro mínimo esperado por operação >= 7%.
#
MAX_MARKET_EXPOSURE = 0.75  # ← UPDATED TO 25% (requirement #5) [Range: 0.03 | 0.15]
MIN_EXPECTED_PROFIT = 0.05  # Lucro mínimo esperado por operação (Regra 7) [Range: 0.03 | 0.20]
...
MAX_RISK_PERCENT  = 1  # Cap PEG ARBIT: investimento PA nunca excede 35% da banca [Range: 0.10 | 0.50]
...
TP_MIN_BID_OVER_ASK  = 0.05   # BID deve ser >= ASK_entrada + 2c para cobrir fees de compra+venda [Range: 0.01 | 0.10]
TP_MIN_PROFIT_PCT    = 0.05   # Lucro líquido mínimo após fees para TP disparar (7% — Regra 7) [Range: 0.03 | 0.20]
...
#
KELLY_ASSUMED_EDGE = 0.15   # ← UPDATED TO 6% (requirement #1)
KELLY_FRACTION     = 0.20   # ← UPDATED TO 1/5 (requirement #2)
KELLY_MAX_RISK_PCT = 0.10   # ← UPDATED TO 5% cap (requirement #2)
...
def fmt_dollar(v: float) -> str:
    """Formata valor monetário: positivo com '+', negativo com '-', zero sem sinal.
    Ex: 4.81 → '$+4.8100', -7.33 → '$-7.3300', 0.0 → '$0.0000'
    """
    if v < 0:
        return f"$-{abs(v):.4f}"
    elif v > 0:
...
def fmt_pct(v: float) -> str:
    """Formata percentagem: positivo com '+', negativo com '-', zero sem sinal.
    Ex: 64.32 → '+64.32%', -141.29 → '-141.29%', 0.0 → '0.00%'
    """
    if v < 0:
        return f"-{abs(v):.2f}%"
    elif v > 0:
...
# =============================================================================
# NEW: EXPLICIT EXPECTED VALUE (EV) — requirement #4
# =============================================================================

def calc_ev(ask: float) -> float:
    """Calculate explicit Expected Value for GAMBLING entry (supporting Kelly).

    EV = p_est * (1 - ask) - (1 - p_est) * ask
    where p_est = min(ask + KELLY_ASSUMED_EDGE, 0.995)

    Logged on every GAMBLING BUY for transparency. Does not gate entry.
    """
    if ask <= 0.0 or ask >= 1.0:
        return 0.0
    p_est = min(ask + KELLY_ASSUMED_EDGE, 0.995)
...
async def logic_loop(m_start: float, m_end: float, meta: dict):
    ...
    async def open_trade(side, trade_type, rstr, risk,
        ...
        # ── v2.0.0: REGRA 7% — Lucro mínimo esperado REMOVIDA COMPLETAMENTE (requirement #6) ────────────
        ...
        # ── EXPLICIT EV (requirement #4) ───────────────────────────────────────
        ev = calc_ev(ask) if trade_type == "GAMBLING" else 0.0
        log_m(trade_type, "BUY",
            ...
            f" | shares={shares:.4f} | risk={risk:.1%}{ext_s} | EV={ev:+.4f}"
        )
        return trade
            ...
            if rem > GAMB_START_REM_S:
                ...
                if not gamb_started_logged:
                    ...
                    log_m("GAMBLING", "START",
                        f"rem={rstr} | activo [300s->0s] | "
                        f"Kelly(edge=6% frac=1/5 cap=5%) | "
